#!/usr/bin/env python
"""
Асинхронная версия YouTube Shorts bulk upload tasks для параллельной работы с несколькими аккаунтами
Скопировано из async_bulk_tasks.py и адаптировано для YouTube Shorts
"""
...
if TYPE_CHECKING:
    from .models import YouTubeShortsBulkUploadTask, YouTubeShortsBulkUploadAccount, YouTubeShortsVideo, YouTubeShortsVideoTitle, YouTubeAccount
...
from .yt_automation import perform_youtube_operations_async
from .browser_utils import BrowserManager, PageUtils, ErrorHandler, NetworkUtils, FileUtils, DebugUtils
from .models import YouTubeShortsBulkUploadTask, YouTubeAccount, YouTubeShortsVideo, YouTubeShortsBulkUploadAccount, YouTubeShortsVideoTitle
...
class AsyncYTTaskRepository:
    """Асинхронный репозиторий для работы с задачами"""
    
    @staticmethod
    @sync_to_async(thread_sensitive=False)
    def get_task(task_id: int) -> YouTubeShortsBulkUploadTask:
        """Получить задачу по ID"""
        return YouTubeShortsBulkUploadTask.objects.select_related().get(id=task_id)
    
    @staticmethod
    @sync_to_async
    def get_account_tasks(task: YouTubeShortsBulkUploadTask) -> List[YouTubeShortsBulkUploadAccount]:
        """Получить аккаунты задачи"""
        return list(task.accounts.select_related('account', 'proxy').all().order_by('account__status'))
    
    @staticmethod
    @sync_to_async
    def get_task_videos(task: YouTubeShortsBulkUploadTask) -> List['YouTubeShortsVideoTitle']:
        """Получить видео задачи"""
        return get_all_task_videos(task)
    
    @staticmethod
    @sync_to_async
    def get_task_titles(task: YouTubeShortsBulkUploadTask) -> List[Any]:
        """Получить заголовки задачи"""
        return get_all_task_titles(task)
    
    @staticmethod
    @sync_to_async(thread_sensitive=False)
    def update_task_status(task: YouTubeShortsBulkUploadTask, status: str, log_message: str = "") -> None:
    ...
    @staticmethod
    @sync_to_async(thread_sensitive=False)
    def update_task_log(task: YouTubeShortsBulkUploadTask, log_message: str) -> None:
    ...
    @staticmethod
    @sync_to_async
    def get_task_properties(task: YouTubeShortsBulkUploadTask) -> Dict[str, Any]:
    ...
    @staticmethod
    @sync_to_async
    def get_account_properties(account_task: YouTubeShortsBulkUploadAccount) -> Dict[str, Any]:
    ...
    @staticmethod
    @sync_to_async
    def get_video_properties(video: 'YouTubeShortsVideo') -> Dict[str, Any]:
        """Получить свойства видео"""
        return {
            'id': video.id,
            'title': video.title or "",
            'description': video.description or "",
            'file_path': video.video_file.path if video.video_file else "",
            'file_size': video.video_file.size if video.video_file else 0,
        }
    ...
    @staticmethod
    @sync_to_async
    def check_videos_empty(videos: List['YouTubeShortsVideoTitle']) -> bool:
        """Проверить, пустой ли список видео"""
        return len(videos) == 0
    
    @staticmethod
    @sync_to_async
    def get_videos_count(videos: List['YouTubeShortsVideoTitle']) -> int:
...
class AsyncYTAccountRepository:
    """Асинхронный репозиторий для работы с аккаунтами"""
    
    @staticmethod
    @sync_to_async(thread_sensitive=False)
    def get_account_details(account: YouTubeAccount, proxy: Optional[Dict] = None) -> Dict[str, Any]:
        """Получить детали аккаунта"""
        details = get_account_details(account, proxy)
        # Добавляем email для YouTube операций
        details['email'] = account.email
        # Добавляем dolphin_profile_id как в Instagram
        details['dolphin_profile_id'] = getattr(account, 'dolphin_profile_id', None)
        return details
    
    @staticmethod
    @sync_to_async(thread_sensitive=False)
    def get_account_proxy(account_task: YouTubeShortsBulkUploadAccount, account: YouTubeAccount) -> Optional[Dict]:
        """Получить прокси аккаунта"""
        return get_account_proxy(account_task, account)
    
    @staticmethod
    @sync_to_async(thread_sensitive=False)
    def update_account_task(account_task: YouTubeShortsBulkUploadAccount, **kwargs) -> None:
...
class AsyncYTFileManager:
    """Асинхронный менеджер файлов"""
    
    def __init__(self, chunk_size: int = AsyncYTConfig.FILE_CHUNK_SIZE):
...
class AsyncYTLogger:
    ...
    def __init__(self, task_id: int, account_id: Optional[int] = None, cache_ns: str = "task_logs", persist_db: bool = True):
        ...
        self.task_repo = AsyncYTTaskRepository()
...
class AsyncYTAccountProcessor:
    ...
    def __init__(self, account_task, task_data, logger: AsyncYTLogger):
        ...
        try:
            ...
            pass
        self.file_manager = AsyncYTFileManager()
        self.account_repo = AsyncYTAccountRepository()
    ...
    async def process(self) -> Tuple[str, int, int]:
        ...
        try:
            ...
            if account.status != 'ACTIVE':
                await self.logger.log('INFO', f"Account {account.email} has status: {account.status} - will attempt processing")
            ...
            if isinstance(result, tuple) and len(result) >= 3:
                ...
                if result_type == 'success' and completed > 0:
                    await self.logger.log('SUCCESS', f"Account {account.email} successfully uploaded {completed} videos in {processing_time:.1f}s")
                elif result_type in ['verification_required', 'phone_verification_required', 'human_verification_required']:
                    await self.logger.log('WARNING', f"Account {account.email} requires verification - processed in {processing_time:.1f}s")
                elif result_type == 'suspended':
                    await self.logger.log('ERROR', f"Account {account.email} is suspended - processed in {processing_time:.1f}s")
                else:
                    await self.logger.log('ERROR', f"Account {account.email} failed to upload videos - processed in {processing_time:.1f}s")
            else:
                await self.logger.log('ERROR', f"Account {account.email} processing failed - processed in {processing_time:.1f}s")
    ...
    async def _prepare_videos_for_account(self) -> List[VideoData]:
        ...
        await self.logger.log('INFO', f"[VIDEO] PREPARING VIDEOS FOR ACCOUNT {self.account_task.account.email}:")
        for i, video in enumerate(videos_for_account):
    ...
    async def _prepare_video_files(self, videos_for_account: List[VideoData]) -> Tuple[List[str], List[str]]:
        ...
        account_username = self.account_task.account.email
    ...
    async def _run_browser_async(self, account_details: Dict, videos: List[VideoData], 
        ...
        import os
        try:
            await self.logger.log('INFO', f"Starting browser for account: {account_details['email']}")
            
            # КРИТИЧЕСКОЕ ДОБАВЛЕНИЕ: Выводим описания видео перед загрузкой в браузере
            await self.logger.log('INFO', f"[TARGET] FINAL VIDEO DESCRIPTIONS FOR {account_details['email']}:")
            ...
            if str(engine).lower() == 'instagrapi':
                ...
                # Используем точно такую же логику как в Instagram
                from .async_impl.dolphin import AsyncDolphinBrowser
                from .yt_automation import perform_youtube_operations_async
                import os
                import asyncio
                ...
                try:
                    ...
                    await self.logger.log('INFO', f"🐬 [YT_DOLPHIN_START] Starting Dolphin Anty browser for YouTube account: {email}")
                    
                    # Pre-flight checks
                    if not video_files_to_upload:
                        await self.logger.log('ERROR', "No video files provided for upload")
                        return ('ERROR', 0, 1)
                    
                    # Initialize Dolphin browser exactly like Instagram
                    dolphin_token = os.environ.get("DOLPHIN_API_TOKEN")
                    if not dolphin_token:
                        await self.logger.log('ERROR', "No Dolphin API token found in environment variables")
                    ...
                    if not dolphin_profile_id:
                        await self.logger.log('ERROR', f"[FAIL] [YT_DOLPHIN_PROFILE] No Dolphin profile ID found for account: {email}")
                        return ('PROFILE_ERROR', 0, 1)
                    
                    await self.logger.log('INFO', f"🔗 [YT_DOLPHIN_PROFILE] Using profile ID: {dolphin_profile_id}")
                    
                    # Connect to profile exactly like Instagram
                    await self.logger.log('INFO', f"🌐 [YT_DOLPHIN_BROWSER] Connecting to browser profile...")
                    page = await dolphin_browser.connect_to_profile_async(dolphin_profile_id, headless=False)
                    
                    if not page:
                        await self.logger.log('ERROR', f"[FAIL] [YT_DOLPHIN_BROWSER] Failed to connect to profile: {dolphin_profile_id}")
                        return ('BROWSER_ERROR', 0, 1)
                    
                    await self.logger.log('INFO', f"[OK] [YT_DOLPHIN_BROWSER] Successfully connected to profile: {dolphin_profile_id}")
                    
                    # Set headers
                    await page.set_extra_http_headers({"Accept-Language": "es-CL,es;q=0.9,en;q=0.8,ru;q=0.7"})
                    ...
                    # Return result
                    if result > 0:
                        await self.logger.log('SUCCESS', f"[YT_SUCCESS] Successfully uploaded {result} videos to YouTube")
                        return ('success', result, 0)
                    else:
                        await self.logger.log('ERROR', f"[YT_FAILED] Failed to upload videos to YouTube")
                        return ('failed', 0, len(videos))
                        
                except Exception as e:
                    await self.logger.log('ERROR', f"[YT_ERROR] YouTube operations failed: {str(e)}")
                    return ('error', 0, len(videos))
                    
                finally:
                    # Cleanup exactly like Instagram
                    if dolphin_browser:
            ...
            error_message = str(e)
            await self.logger.log('ERROR', f"Browser task failed for account {account_details['email']}: {error_message}")
            
            # Обрабатываем специальные случаи верификации и блокировок
            if "PHONE_VERIFICATION_REQUIRED" in error_message:
                await self.logger.log('WARNING', f"[PHONE] Phone verification required for account {account_details['email']}")
                return 'phone_verification_required', 0, 1
            elif "HUMAN_VERIFICATION_REQUIRED" in error_message:
                await self.logger.log('WARNING', f"[BOT] Human verification required for account {account_details['email']}")
                return 'human_verification_required', 0, 1
            elif "SUSPENDED" in error_message:
                await self.logger.log('WARNING', f"[BLOCK] Account {account_details['email']} is suspended")
...
class AsyncYTTaskCoordinator:
    """Координатор для асинхронного выполнения задач"""
    
    def __init__(self, task_id: int):
        self.task_id = task_id
        self.task_repo = AsyncYTTaskRepository()
        self.account_semaphore = asyncio.Semaphore(AsyncYTConfig.MAX_CONCURRENT_ACCOUNTS)
        self.start_time = None
        self.end_time = None
    ...
    async def run(self) -> bool:
        ...
        try:
            ...
            logger = AsyncYTLogger(self.task_id)
            set_async_logger(logger)
            ...
            if use_rounds:
                ...
                for round_index, video_data in enumerate(all_video_datas, start=1):
                    ...
                    for account_task in accounts_order:
                        ...
                        processor = AsyncYTAccountProcessor(account_task, single_video_task_data, logger)
                ...
                for i, account_task in enumerate(account_tasks):
                    processor = AsyncYTAccountProcessor(account_task, task_data, logger)
                ...
                max_concurrent = AsyncYTConfig.MAX_CONCURRENT_ACCOUNTS
    ...
    async def _create_task_data(self, task: YouTubeShortsBulkUploadTask, account_tasks: List[YouTubeShortsBulkUploadAccount], 
                               all_videos: List['YouTubeShortsVideoTitle'], all_titles: List[str]) -> TaskData:
    ...
    async def _process_account_with_semaphore(self, processor: AsyncYTAccountProcessor, 
                                            account_task: YouTubeShortsBulkUploadAccount, start_delay: float = 0.0) -> Tuple[str, int, int]:
        ...
        async with self.account_semaphore:
            ...
            delay = random.uniform(AsyncYTConfig.ACCOUNT_DELAY_MIN, AsyncYTConfig.ACCOUNT_DELAY_MAX)
    ...
    async def _process_results(self, results: List, account_tasks: List[YouTubeShortsBulkUploadAccount], 
                             logger: AsyncYTLogger) -> None:
                ...
                if result_type == 'success' and completed > 0:
                    ...
                    try:
                        if i < len(account_tasks):
                            ...
                            await AsyncYTAccountRepository.update_account_task(account_task, status=TaskStatus.COMPLETED)
                    ...
                    if i < len(account_tasks):
                        account_task = account_tasks[i]
                        await AsyncYTAccountRepository.update_account_task(
                            account_task, 
                            status=result_type.upper()
                        )
                        await logger.log('INFO', f"Updated account {account_task.account.email} status to {result_type.upper()}")
                elif result_type == 'suspended':
                    ...
                    if i < len(account_tasks):
                        ...
                        await logger.log('INFO', f"Updated account {account_task.account.email} status to SUSPENDED")
                else:
                    failed_accounts += 1
                    ...
                    try:
                        if i < len(account_tasks):
                            ...
                            await AsyncYTAccountRepository.update_account_task(account_task, status=TaskStatus.FAILED)
                            _tu.update_account_task(account_task)
                    except Exception:
                        pass
            else:
    ...
    async def _finalize_task(self, task: YouTubeShortsBulkUploadTask, logger: AsyncYTLogger) -> None:
        """Завершает задачу"""
        current_time = timezone.now()
        timestamp = current_time.strftime("%Y-%m-%d %H:%M:%S")
    ...
    async def _cleanup_original_video_files(self, task: YouTubeShortsBulkUploadTask, logger: AsyncYTLogger) -> int:
                ...
                try:
                    ...
                    if file_path and os.path.exists(file_path):
                        ...
                        def is_file_safe_to_delete():
                            ...
                            # Проверяем, есть ли другие YouTubeShortsVideo объекты с таким же файлом в активных задачах
                            from .models import YouTubeShortsVideo, YouTubeShortsBulkUploadTask
                            
                            other_videos_with_same_file = YouTubeShortsVideo.objects.filter(
...
async def run_async_yt_shorts_task(task_id: int) -> bool:
    """
    ...
    def signal_handler(signum, frame):
        ...
        try:
            ...
            task = YouTubeShortsBulkUploadTask.objects.get(id=task_id)
            current_time = timezone.now()
            timestamp = current_time.strftime("%Y-%m-%d %H:%M:%S")
    ...
    try:
        ...
        coordinator = AsyncYTTaskCoordinator(task_id)
        ...
        try:
            ...
            task_repo = AsyncYTTaskRepository()
        ...
        try:
            ...
            task = await task_repo.get_task(task_id)
            coordinator = AsyncYTTaskCoordinator(task_id)
            deleted_count = await coordinator._cleanup_original_video_files(task, AsyncYTLogger(task_id))
        ...
        try:
            await AsyncYTTaskRepository.close_django_connections()
        except Exception as e:
            print(f"Warning: Failed to close Django connections: {str(e)}")

def run_async_yt_shorts_task_sync(task_id: int) -> bool:
    """
    ...
    try:
        ...
        def run_in_thread():
            ...
            try:
                ...
                result = new_loop.run_until_complete(run_async_yt_shorts_task(task_id))
...
def set_async_config(**kwargs) -> None:
    ...
    for key, value in kwargs.items():
        if hasattr(AsyncYTConfig, key.upper()):
            setattr(AsyncYTConfig, key.upper(), value)
            print(f"[TOOL] Set {key.upper()} = {value}")

def get_async_config() -> Dict[str, Any]:
    """Получить текущую конфигурацию"""
    return {
        'max_concurrent_accounts': AsyncYTConfig.MAX_CONCURRENT_ACCOUNTS,
        'max_concurrent_videos': AsyncYTConfig.MAX_CONCURRENT_VIDEOS,
        'account_delay_min': AsyncYTConfig.ACCOUNT_DELAY_MIN,
        'account_delay_max': AsyncYTConfig.ACCOUNT_DELAY_MAX,
        'retry_attempts': AsyncYTConfig.RETRY_ATTEMPTS,
        'health_check_interval': AsyncYTConfig.HEALTH_CHECK_INTERVAL,
    } 