...
async def run_dolphin_browser_async(account_details: Dict, videos: List, video_files_to_upload: List[str],
    ...
    try:
        ...
        print(f"🐬 [ASYNC_DOLPHIN_START] Starting Dolphin Anty browser for account: {username}")
        
        # Pre-flight checks
        if not video_files_to_upload:
            error_msg = "No video files provided for upload"
            print(f"❌ [ASYNC_DOLPHIN_ERROR] {error_msg}")
            return ("ERROR", 0, 1)
        ...
        print(f"🐬 [ASYNC_DOLPHIN_CONFIG] Using Dolphin API host: {dolphin_api_host}")
        
        dolphin = DolphinAnty(api_key=dolphin_token, local_api_base=dolphin_api_host)
        
        # Enhanced authentication with retry logic
        print(f"🔐 [ASYNC_DOLPHIN_AUTH] Authenticating with Dolphin Anty API...")
        auth_attempts = 0
        max_auth_attempts = 3
        
        while auth_attempts < max_auth_attempts:
            if await authenticate_dolphin_async(dolphin):
                print(f"✅ [ASYNC_DOLPHIN_AUTH] Authentication successful on attempt {auth_attempts + 1}")
                break
            else:
                auth_attempts += 1
                if auth_attempts < max_auth_attempts:
                    print(f"⚠️ [ASYNC_DOLPHIN_AUTH] Authentication failed, retrying... (attempt {auth_attempts}/{max_auth_attempts})")
                    await asyncio.sleep(random.uniform(2, 5))
                else:
                    error_msg = "Failed to authenticate with Dolphin Anty API after multiple attempts"
                    print(f"❌ [ASYNC_DOLPHIN_AUTH] {error_msg}")
                    return ("DOLPHIN_ERROR", 0, 1)
        
        # Get profile with validation
        print(f"📋 [ASYNC_DOLPHIN_PROFILE] Retrieving Dolphin profile for account: {username}")
        ...
        if not dolphin_profile_id:
            ...
            print(f"❌ [ASYNC_DOLPHIN_PROFILE] {error_msg}")
            return ("PROFILE_ERROR", 0, 1)
        
        print(f"🔗 [ASYNC_DOLPHIN_PROFILE] Found profile ID: {dolphin_profile_id}")
        
        # Connect to browser profile
        print("🌐 [ASYNC_DOLPHIN_BROWSER] Connecting to browser profile...")
        ...
        if not page:
            ...
            print(f"❌ [ASYNC_DOLPHIN_BROWSER] {error_msg}")
            return ("BROWSER_ERROR", 0, 1)
        
        print("✅ [ASYNC_DOLPHIN_BROWSER] Successfully connected to profile: " + str(dolphin_profile_id))
        
        # Perform Instagram operations
        print(f"🔍 [ASYNC_INSTAGRAM_PREP] Preparing Instagram operations for {len(videos)} videos")
        ...
        if result:  # result теперь содержит количество загруженных видео
            ...
            if uploaded_count > 0:
                print(f"✅ [ASYNC_SUCCESS] Successfully uploaded {uploaded_count}/{len(videos)} videos")
                
                # ENHANCED: Update last_used for successful uploads
                try:
                    await update_account_last_used_async(username)
                    print(f"✅ [ASYNC_SUCCESS] Updated last_used for account: {username}")
                except Exception as last_used_error:
                    print(f"⚠️ [ASYNC_SUCCESS] Failed to update last_used: {str(last_used_error)}")
                
                return ("SUCCESS", uploaded_count, failed_count)
            else:
                print(f"❌ [ASYNC_FAIL] No videos were uploaded despite successful operations")
                return ("ERROR", 0, len(videos))
        else:
            print(f"❌ [ASYNC_FAIL] Instagram operations failed")
            
            # ENHANCED: Update last_used even for failed operations (account was used)
            try:
                await update_account_last_used_async(username)
                print(f"✅ [ASYNC_FAIL] Updated last_used for failed account: {username}")
            except Exception as last_used_error:
                print(f"⚠️ [ASYNC_FAIL] Failed to update last_used: {str(last_used_error)}")
            
            return ("ERROR", 0, len(videos))
    
    except Exception as e:
        error_message = str(e)
        print(f"💥 [ASYNC_DOLPHIN_EXCEPTION] Critical error in Dolphin browser for account {username}: {error_message}")
        
        # Handle different error types with proper account status updates
        if "PHONE_VERIFICATION_REQUIRED" in error_message:
            print(f"📱 [ASYNC_VERIFICATION_PHONE] Phone verification required for account: {username}")
            ...
            print(f"🤖 [ASYNC_VERIFICATION_HUMAN] Human verification required for account: {username}")
        ...
        print("[ASYNC_CLEANUP] Starting comprehensive browser cleanup")
        try:
            if dolphin_browser:
                await dolphin_browser.cleanup_async()
                print("[ASYNC_CLEANUP] Browser cleanup completed")
            else:
                print("[ASYNC_CLEANUP] Browser objects not initialized, skipping cleanup")
        except Exception as cleanup_error:
            print(f"[ASYNC_CLEANUP] Error during cleanup: {str(cleanup_error)}")
...
async def update_account_status_async(username: str, status: str, account_task_id: int):
    ...
    try:
        ...
        print(f"💾 [ASYNC_DATABASE] Updating account {username} status to {status}")
        ...
        def update_instagram_account():
            try:
                ...
                print(f"💾 [ASYNC_DATABASE] Instagram account {username} not found")
                return False
        ...
        def update_bulk_account():
            try:
                ...
                print(f"💾 [ASYNC_DATABASE] BulkUploadAccount with ID {account_task_id} not found")
                return False
        ...
        if instagram_updated:
            print(f"💾 [ASYNC_DATABASE] Updated Instagram account {username} status to {status}")
        if bulk_updated:
            print(f"💾 [ASYNC_DATABASE] Updated bulk account task {account_task_id} status to {status}")
            
    except Exception as db_error:
        print(f"💾 [ASYNC_DATABASE_ERROR] Failed to update account status: {str(db_error)}")
...
async def authenticate_dolphin_async(dolphin) -> bool:
    ...
    try:
        ...
        print("🔐 [ASYNC_DOLPHIN_AUTH] Authenticating with Dolphin Anty API...")
        ...
        if not auth_result:
            print("❌ [ASYNC_DOLPHIN_AUTH] Failed to authenticate with Dolphin Anty API")
            return False
        
        print("✅ [ASYNC_DOLPHIN_AUTH] Successfully authenticated with Dolphin Anty API")
        
        # Check application status - exact copy from sync
        print("🔍 [ASYNC_DOLPHIN_AUTH] Checking Dolphin Anty application status...")
        ...
        if not dolphin_status["app_running"]:
            ...
            print(f"❌ [ASYNC_DOLPHIN_AUTH] Dolphin Anty application is not running: {error_msg}")
            return False
        
        print("✅ [ASYNC_DOLPHIN_AUTH] Dolphin Anty application is running and ready")
        return True
        
    except Exception as e:
        print(f"❌ [ASYNC_DOLPHIN_AUTH] Authentication error: {str(e)}")
        return False
...
class AsyncDolphinBrowser:
    ...
    def __init__(self, dolphin_api_token: str = None):
        if dolphin_api_token:
            print(f"✅ [ASYNC_DOLPHIN_INIT] Initializing Dolphin Anty with API token")
        ...
        if not dolphin_api_host.endswith("/v1.0"):
            ...
            print(f"🐬 [ASYNC_DOLPHIN_INIT] Using Dolphin API host: {dolphin_api_host}")
            
            from bot.src.instagram_uploader.dolphin_anty import DolphinAnty
            self.dolphin = DolphinAnty(api_key=dolphin_api_token, local_api_base=dolphin_api_host)
        else:
            print(f"❌ [ASYNC_DOLPHIN_INIT] No Dolphin API token provided - cannot initialize DolphinAnty")
    ...
    async def connect_to_profile_async(self, profile_id: str, headless: bool = False):
        ...
        try:
            ...
            print(f"🔄 [ASYNC_BROWSER] [Step 1/5] Starting Dolphin Anty profile: {profile_id} (headless: {headless})")
            ...
            if not success or not automation_data:
                print(f"❌ [ASYNC_BROWSER] Failed to start Dolphin profile: {profile_id}")
                return None
            ...
            if not port or not ws_endpoint:
                print(f"❌ [ASYNC_BROWSER] Invalid automation data from Dolphin API:")
                print(f"   Port: {port}")
                print(f"   WS Endpoint: {ws_endpoint}")
                print(f"   Full data: {automation_data}")
                return None
            ...
            print(f"🔗 [ASYNC_BROWSER] WebSocket URL: {ws_url}")
                            
            # Initialize Playwright
            print(f"🔄 [ASYNC_BROWSER] [Step 2/5] Initializing Playwright...")
            self.playwright = await async_playwright().start()
                            
            # Connect to browser
            print(f"🔄 [ASYNC_BROWSER] [Step 3/5] Connecting to Dolphin browser via WebSocket...")
            try:
                self.browser = await self.playwright.chromium.connect_over_cdp(ws_url)
                print(f"✅ [ASYNC_BROWSER] Successfully connected to browser using CDP")
            except Exception as connect_error:
                print(f"❌ [ASYNC_BROWSER] Failed to connect via CDP: {connect_error}")
                print(f"   Make sure Dolphin profile {profile_id} is running")
                print(f"   WebSocket URL: {ws_url}")
                return None
                            
            # Use the default context (using contexts property like in working code)
            print(f"🔄 [ASYNC_BROWSER] [Step 4/5] Getting browser context...")
            if self.browser.contexts:
                self.context = self.browser.contexts[0]
                print(f"✅ [ASYNC_BROWSER] Using existing browser context")
            else:
                print(f"❌ [ASYNC_BROWSER] No browser contexts available")
                return None
            
            # Get the page
            print(f"🔄 [ASYNC_BROWSER] [Step 5/5] Getting page...")
            if self.context.pages:
                self.page = self.context.pages[0]
                print(f"✅ [ASYNC_BROWSER] Using existing page")
            else:
                self.page = await self.context.new_page()
                print(f"✅ [ASYNC_BROWSER] Created new page")
            
            print(f"✅ [ASYNC_BROWSER] Successfully connected to Dolphin profile: {profile_id}")
            return self.page
        
        except Exception as e:
            print(f"❌ [ASYNC_BROWSER] Error connecting to profile {profile_id}: {str(e)}")
    ...
    async def cleanup_async(self):
        ...
        try:
            print("🔒 [ASYNC_BROWSER_CLEANUP] Starting browser cleanup...")
            
            # Close page first
            if self.page:
                try:
                    await self.page.close()
                    print("✅ [ASYNC_BROWSER_CLEANUP] Page closed")
                except Exception as e:
                    print(f"⚠️ [ASYNC_BROWSER_CLEANUP] Error closing page: {str(e)}")
                finally:
                    self.page = None
            ...
            if self.browser:
                try:
                    ...
                    print("✅ [ASYNC_BROWSER_CLEANUP] Browser closed")
                except Exception as e:
                    print(f"⚠️ [ASYNC_BROWSER_CLEANUP] Error closing browser: {str(e)}")
                finally:
                    self.browser = None
            ...
            if self.playwright:
                try:
                    ...
                    print("✅ [ASYNC_BROWSER_CLEANUP] Playwright stopped")
                except Exception as e:
                    print(f"⚠️ [ASYNC_BROWSER_CLEANUP] Error stopping playwright: {str(e)}")
                finally:
                    self.playwright = None
            ...
            if self.dolphin and self.dolphin_profile_id:
                try:
                    ...
                    print("✅ [ASYNC_BROWSER_CLEANUP] Dolphin profile stopped")
                except Exception as e:
                    print(f"⚠️ [ASYNC_BROWSER_CLEANUP] Error stopping Dolphin profile: {str(e)}")
            ...
            print("✅ [ASYNC_BROWSER_CLEANUP] Cleanup completed successfully")
            
        except Exception as e:
            print(f"❌ [ASYNC_BROWSER_CLEANUP] Critical cleanup error: {str(e)}")
...
async def get_dolphin_profile_id_async(username: str) -> str:
    ...
    try:
        ...
        print(f"❌ [ASYNC_DOLPHIN_PROFILE] Error getting dolphin profile ID for {username}: {str(e)}")
        return None
...
async def perform_instagram_operations_async(page, account_details: Dict, videos: List, video_files_to_upload: List[str]) -> bool:
    ...
    try:
        ...
        print("🔍 [ASYNC_VERIFICATION] Checking account status after login...")
        try:
            await check_post_login_verifications_async(page, account_details)
            print("✅ [ASYNC_VERIFICATION] Account status check completed - no issues detected")
        except Exception as verification_error:
            error_message = str(verification_error)
            print(f"❌ [ASYNC_VERIFICATION] Account status issue detected: {error_message}")
            ...
            if ("PHONE_VERIFICATION_REQUIRED:" in error_message or 
                ...
                print(f"⚠️ [ASYNC_VERIFICATION] Non-critical verification error: {error_message}")
        ...
        if len(videos) != len(video_files_to_upload):
            print(f"[ASYNC_UPLOAD] ⚠️ Mismatch: {len(videos)} videos vs {len(video_files_to_upload)} files")
            print(f"[ASYNC_UPLOAD] 🔍 Videos: {[getattr(v, 'video_file', 'NO_FILE').name if hasattr(v, 'video_file') else 'NO_ATTR' for v in videos]}")
            print(f"[ASYNC_UPLOAD] 🔍 Files: {[os.path.basename(f) for f in video_files_to_upload]}")
        ...
        for i, video_file_path in enumerate(video_files_to_upload, 1):
            try:
                ...
                if i <= len(videos):
                    ...
                    print(f"[ASYNC_UPLOAD] ⚠️ No video object for file {i}, using None")
                    video_obj = None
                ...
                if not await navigate_to_upload_with_human_behavior_async(page):
                    print(f"[ASYNC_FAIL] Could not navigate to upload page for video {i}")
                    continue
                
                # Upload video
                if await upload_video_with_human_behavior_async(page, video_file_path, video_obj):
                    uploaded_videos += 1
                    print(f"[ASYNC_SUCCESS] Video {i}/{len(video_files_to_upload)} uploaded successfully")
                    if video_obj and hasattr(video_obj, 'uploaded'):
                        video_obj.uploaded = True
                else:
                    print(f"[ASYNC_FAIL] Failed to upload video {i}/{len(video_files_to_upload)}")
                ...
                print(f"[ASYNC_FAIL] Error uploading video {i}: {str(e)}")
                import traceback
                print(f"[ASYNC_FAIL] 🔍 Traceback: {traceback.format_exc()}")
                continue
        
        # КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: Навигация только после подтверждения успешной загрузки
        if uploaded_videos > 0:
            print(f"[ASYNC_SUCCESS] ✅ {uploaded_videos} videos uploaded successfully, performing cleanup")
            # Final cleanup только после успешной загрузки
            await perform_final_cleanup_async(page, account_details['username'])
        else:
            print(f"[ASYNC_FAIL] ❌ No videos were uploaded, skipping cleanup to preserve upload state")
        ...
        if "PHONE_VERIFICATION_REQUIRED" in error_message:
            print(f"Phone verification required: {error_message}")
            raise e  # Re-raise to be caught by run_dolphin_browser_async
        elif "HUMAN_VERIFICATION_REQUIRED" in error_message:
            print(f"Human verification required: {error_message}")
            raise e  # Re-raise to be caught by run_dolphin_browser_async
        elif "SUSPENDED" in error_message:
            print(f"Account suspended: {error_message}")
            raise e  # Re-raise to be caught by run_dolphin_browser_async
        else:
            print(f"Error in Instagram operations: {error_message}")
            return False

async def init_human_behavior_async(page):
    """Initialize human behavior for async operations - ПОЛНАЯ КОПИЯ sync логики"""
    try:
        print("[ASYNC_HUMAN] Initializing human behavior system...")
        # Human behavior initialization logic would go here
        # For now, just a placeholder
        await asyncio.sleep(1)
        print("[ASYNC_HUMAN] ✅ Human behavior initialized")
    except Exception as e:
        print(f"[ASYNC_HUMAN] ❌ Error initializing human behavior: {str(e)}")
...
async def handle_login_flow_async(page, account_details: Dict) -> bool:
    ...
    try:
        print("🔑 [ASYNC_LOGIN] Starting enhanced login flow...")
        
        # Wait for page to be fully interactive before checking login status
        print("⏳ [ASYNC_LOGIN] Ensuring page is fully loaded before login check...")
        ...
        if logged_in_status == "SUSPENDED":
            print(f"🚫 [ASYNC_LOGIN] Account is SUSPENDED - cannot proceed with login")
            raise Exception(f"SUSPENDED: Account suspended by Instagram")
        elif logged_in_status:
            print(f"✅ [ASYNC_LOGIN] Already logged in! Skipping login process")
            
            # Still need to check for post-login verification requirements
            print("🔍 [ASYNC_LOGIN] Checking for verification requirements...")
            await check_post_login_verifications_async(page, account_details)
            return True
        
        print("🔑 [ASYNC_LOGIN] User not logged in - need to login to Instagram")
        ...
        if login_result == "SUSPENDED":
            print("🚫 [ASYNC_LOGIN] Account suspended detected during login")
            raise Exception(f"SUSPENDED: Account suspended by Instagram")
        elif not login_result:
            print("❌ [ASYNC_LOGIN] Failed to login to Instagram")
            return False
        
        print("✅ [ASYNC_LOGIN] Login completed successfully")
        ...
        print("✅ [ASYNC_LOGIN] Login flow completed successfully")
        return True
...
async def check_if_already_logged_in_async(page, selectors):
    ...
    print(f"🔍 [ASYNC_LOGIN] Current URL: {current_url}")
    ...
    try:
        ...
        for keyword in suspension_keywords:
            if keyword in page_text.lower():
                print(f"🚫 [ASYNC_LOGIN] Account suspension detected from text: '{keyword}'")
                print(f"🚫 [ASYNC_LOGIN] Page text sample: '{page_text[:200]}...'")
                return "SUSPENDED"
                
    except Exception as e:
        print(f"🚫 [ASYNC_LOGIN] Could not check page text for suspension: {str(e)}")
    ...
    if url_indicates_suspension:
        print(f"🚫 [ASYNC_LOGIN] Account suspension also detected from URL: {current_url}")
        return "SUSPENDED"
    ...
    for indicator in selectors.LOGIN_FORM_INDICATORS:
        try:
            ...
            if element and await element.is_visible():
                ...
                print(f"🔍 [ASYNC_LOGIN] Found login form element: {indicator}")
        except:
            continue
    
    if login_form_present:
        print(f"🔍 [ASYNC_LOGIN] Login form detected with elements: {found_login_elements[:3]}")
        return False
    ...
    print("🔍 [ASYNC_LOGIN] No login form found, checking for logged-in indicators...")
            ...
            if element and await element.is_visible():
                ...
                try:
                    ...
                    if any(keyword in combined_text for keyword in exclusion_keywords):
                        print(f"🔍 [ASYNC_LOGIN] Skipping element {i+1} (contains account creation text): '{element_text.strip()}'")
                        continue
                    
                    logged_in_found = True
                    found_indicators.append(indicator)
                    print(f"✅ [ASYNC_LOGIN] Found logged-in indicator {i+1}: {indicator}")
                    
                    if element_text.strip():
                        print(f"✅ [ASYNC_LOGIN] Element text: '{element_text.strip()}'")
                    ...
                    found_indicators.append(indicator)
                    print(f"✅ [ASYNC_LOGIN] Found logged-in indicator {i+1}: {indicator}")
                    print(f"🔍 [ASYNC_LOGIN] Could not analyze element text: {str(e)}")
                ...
                if any(strong_keyword in indicator.lower() for strong_keyword in [
                    ...
                    print(f"✅ [ASYNC_LOGIN] Strong logged-in indicator found: {indicator}")
                    break
                    
        except Exception as e:
            print(f"🔍 [ASYNC_LOGIN] Error checking indicator {indicator}: {str(e)}")
            continue
    
    if logged_in_found:
        print(f"✅ [ASYNC_LOGIN] Already logged in! Found {len(found_indicators)} indicators: {found_indicators[:5]}")
        
        # Additional verification - check page title (EXACT COPY from sync)
        try:
            page_title = await page.title()
            print(f"✅ [ASYNC_LOGIN] Page title: '{page_title}'")
            
            # Instagram main page usually has "Instagram" in title
            if "instagram" in page_title.lower():
                print("✅ [ASYNC_LOGIN] Page title confirms Instagram main page")
            
        except Exception as e:
            print(f"🔍 [ASYNC_LOGIN] Could not get page title: {str(e)}")
    ...
    print("🔍 [ASYNC_LOGIN] No logged-in indicators found - user needs to login")
    return False
...
async def simulate_human_mouse_movement_async(page):
    ...
    try:
        ...
        print(f"[ASYNC_HUMAN] Mouse movement simulation error: {str(e)}")
...
async def navigate_to_upload_with_human_behavior_async(page):
    ...
    try:
        print("[ASYNC_UPLOAD] 🚀 Starting enhanced navigation to upload interface")
        
        # ENHANCED: Initial page readiness check before starting navigation
        print("[ASYNC_UPLOAD] 🔍 Performing initial page readiness check...")
        initial_ready = await wait_for_page_ready_async(page, max_wait_time=15.0)
        if not initial_ready:
            print("[ASYNC_UPLOAD] ⚠️ Initial page readiness check failed, but proceeding...")
        else:
            print("[ASYNC_UPLOAD] ✅ Initial page readiness check passed")
        
        # Debug: Log current page state (ПОЛНАЯ КОПИЯ из sync)
        try:
            current_url = page.url
            page_title = await page.title()
            print(f"[ASYNC_UPLOAD] 📍 Current page: {current_url}")
            print(f"[ASYNC_UPLOAD] 📄 Page title: {page_title}")
        except Exception as debug_error:
            print(f"[ASYNC_UPLOAD] Could not get page info: {str(debug_error)}")
        ...
        if success:
            print("[ASYNC_UPLOAD] ✅ Successfully navigated to upload interface")
            
            # Additional verification - check for file input immediately (ПОЛНАЯ КОПИЯ из sync)
            try:
                final_url = page.url
                print(f"[ASYNC_UPLOAD] 📍 Final URL: {final_url}")
                
                # Check if we can see file input elements immediately (ПОЛНАЯ КОПИЯ из sync)
                file_inputs = await page.query_selector_all('input[type="file"]')
                print(f"[ASYNC_UPLOAD] 📁 Found {len(file_inputs)} file input elements")
                ...
                print(f"[ASYNC_UPLOAD] 📁 Found {len(semantic_file_inputs)} semantic file input elements")
                ...
                if len(file_inputs) > 0 or len(semantic_file_inputs) > 0:
                    for i, inp in enumerate(file_inputs + semantic_file_inputs):
                        try:
                            ...
                            print(f"[ASYNC_UPLOAD] Input {i+1}: accept='{inp_accept}', visible={inp_visible}, class='{inp_class[:30]}'")
                        except:
                            pass
                    
                    print("[ASYNC_UPLOAD] ✅ File input elements are ready for upload")
                else:
                    print("[ASYNC_UPLOAD] ⚠️ No file input elements found after navigation")
                            
            except Exception as verify_error:
                print(f"[ASYNC_UPLOAD] Could not verify upload interface: {str(verify_error)}")
        else:
            print("[ASYNC_UPLOAD] ❌ Failed to navigate to upload interface")
        
        return success
        
    except Exception as e:
        print(f"[ASYNC_UPLOAD] ❌ Navigation failed: {str(e)}")
        return False

async def wait_for_page_ready_async(page, max_wait_time=30.0) -> bool:
    """Wait for page to be fully ready - OPTIMIZED VERSION"""
    try:
        print(f"[ASYNC_READY] ⏳ Quick page readiness check (max {max_wait_time}s)...")
        ...
        while time.time() - start_time < max_wait_time:
            try:
                ...
                if ready_state != "complete":
                    print(f"[ASYNC_READY] ⏳ Document not ready: {ready_state}")
                ...
                if not main_element:
                    print(f"[ASYNC_READY] ⏳ Main interface not found")
                ...
                print(f"[ASYNC_READY] ⚠️ Error checking page state: {str(e)}")
                await asyncio.sleep(0.5)
                continue
        
        print(f"[ASYNC_READY] ❌ Page not ready after {max_wait_time}s")
        return False
        
    except Exception as e:
        print(f"[ASYNC_READY] ❌ Error in wait_for_page_ready_async: {str(e)}")
        return False

async def navigate_to_upload_core_async(page):
    """Navigate to upload page with human behavior - OPTIMIZED VERSION"""
    try:
        print("[ASYNC_UPLOAD] 🧠 Starting navigation to upload page")
        
        # Quick page readiness check
        page_ready = await wait_for_page_ready_async(page, max_wait_time=10.0)  # Reduced timeout
        if not page_ready:
            print("[ASYNC_UPLOAD] ⚠️ Page not ready, but proceeding with navigation...")
        ...
        if not upload_button:
            print("[ASYNC_UPLOAD] ⚠️ Upload button not found, trying alternative navigation...")
        ...
        try:
            ...
            if not is_visible:
                ...
                if not upload_button:
                    print("[ASYNC_UPLOAD] ❌ Upload button still not found after retry")
                    return await navigate_to_upload_alternative_async(page)
        except Exception as e:
            print(f"[ASYNC_UPLOAD] ⚠️ Error checking upload button: {str(e)}")
        
        # Click upload button
        await click_element_with_behavior_async(page, upload_button, "UPLOAD_BTN")
        
        # Wait and observe page changes
        print("[ASYNC_UPLOAD] 👀 Observing page changes...")
        await simulate_page_scan_async(page)
        ...
        if not success:
            print("[ASYNC_UPLOAD] ⚠️ Standard navigation failed, trying alternative...")
            return await navigate_to_upload_alternative_async(page)
        ...
        print(f"[ASYNC_UPLOAD] ❌ Navigation failed: {str(e)}")
        print("[ASYNC_UPLOAD] 🔄 Trying alternative navigation method...")
        return await navigate_to_upload_alternative_async(page)
    ...
    try:
        for selector in selectors:
            try:
                ...
                if element and await element.is_visible():
                    print(f"[ASYNC_UPLOAD] ✅ Found {element_name} with selector: {selector}")
                    return element
            except Exception as e:
                print(f"[ASYNC_UPLOAD] Selector failed: {selector} - {str(e)}")
                continue
        
        print(f"[ASYNC_UPLOAD] ❌ {element_name} not found with any selector")
        return None
        
    except Exception as e:
        print(f"[ASYNC_UPLOAD] ❌ Error finding {element_name}: {str(e)}")
        return None
...
async def click_element_with_behavior_async(page, element, element_name):
    ...
    try:
        ...
        try:
            ...
            if not is_visible:
                print(f"[ASYNC_UPLOAD] ⚠️ {element_name} is not visible, waiting...")
                await asyncio.sleep(2.0 + random.uniform(-0.5, 0.5))
                is_visible = await element.is_visible()
                
            if not is_enabled:
                print(f"[ASYNC_UPLOAD] ⚠️ {element_name} is not enabled, waiting...")
                await asyncio.sleep(2.0 + random.uniform(-0.5, 0.5))
                is_enabled = await element.is_enabled()
                
            if not is_visible or not is_enabled:
                print(f"[ASYNC_UPLOAD] ❌ {element_name} still not ready (visible: {is_visible}, enabled: {is_enabled})")
                raise Exception(f"Element {element_name} not ready for click")
                
        except Exception as readiness_error:
            print(f"[ASYNC_UPLOAD] ⚠️ Error checking element readiness: {str(readiness_error)}")
        ...
        # Click element
        await element.click()
        print(f"[ASYNC_UPLOAD] ✅ {element_name} clicked successfully")
        
        # Brief pause after click
        await asyncio.sleep(random.uniform(0.5, 1.0))
        
    except Exception as e:
        print(f"[ASYNC_UPLOAD] ❌ Error clicking {element_name}: {str(e)}")
        raise
...
async def handle_post_upload_click_async(page) -> bool:
    ...
    try:
        print("[ASYNC_UPLOAD] 🚀 Starting enhanced post-upload click handling...")
        ...
        print(f"[ASYNC_UPLOAD] ⏳ Waiting {initial_wait:.1f}s for interface response (enhanced timeout)...")
        await asyncio.sleep(initial_wait)
        
        # Check for file dialog first
        if await check_for_file_dialog_async(page):
            print("[ASYNC_UPLOAD] 📁 File dialog opened directly - no menu needed")
            return True
        
        # Check for dropdown menu
        if await check_for_dropdown_menu_async(page):
            print("[ASYNC_UPLOAD] 📋 Dropdown menu detected - selecting post option")
        ...
        print(f"[ASYNC_UPLOAD] ⏳ Waiting additional {additional_wait:.1f}s (enhanced timeout)...")
        await asyncio.sleep(additional_wait)
        
        if await check_for_file_dialog_async(page):
            print("[ASYNC_UPLOAD] 📁 File dialog appeared after enhanced delay")
            return True
        
        if await check_for_dropdown_menu_async(page):
            print("[ASYNC_UPLOAD] 📋 Menu appeared after enhanced delay")
            return await click_post_option_async(page)
        
        # ENHANCED: Try broader detection with retry logic
        print("[ASYNC_UPLOAD] ⚠️ Neither menu nor file dialog detected, trying enhanced broader detection...")
        ...
        print("[ASYNC_UPLOAD] 🔄 First attempt failed, trying retry with page refresh...")
        return await retry_upload_with_page_refresh_async(page)
        
    except Exception as e:
        print(f"[ASYNC_UPLOAD] ❌ Error handling post-upload click: {str(e)}")
        return False

async def retry_upload_with_page_refresh_async(page) -> bool:
    """Retry upload process with page refresh when dialog doesn't load"""
    try:
        print("[ASYNC_UPLOAD] 🔄 Starting retry with page refresh...")
        
        # Save current URL for navigation back
        current_url = page.url
        print(f"[ASYNC_UPLOAD] 📍 Current URL: {current_url}")
        
        # Refresh the page
        print("[ASYNC_UPLOAD] 🔄 Refreshing page...")
        await page.reload(wait_until="domcontentloaded", timeout=30000)
        
        # Wait for page to fully load
        refresh_wait = 5.0 + random.uniform(-1.0, 1.0)
        print(f"[ASYNC_UPLOAD] ⏳ Waiting {refresh_wait:.1f}s for page to load after refresh...")
        ...
        print("[ASYNC_UPLOAD] 🔍 Looking for upload button after refresh...")
        upload_button = await find_element_with_selectors_async(page, SelectorConfig.UPLOAD_BUTTON, "UPLOAD_BTN_RETRY")
        
        if not upload_button:
            print("[ASYNC_UPLOAD] ⚠️ Upload button not found after refresh, trying alternative navigation...")
            return await navigate_to_upload_alternative_async(page)
        
        # Click upload button again
        print("[ASYNC_UPLOAD] 🖱️ Clicking upload button after refresh...")
        ...
        print(f"[ASYNC_UPLOAD] ⏳ Waiting {retry_wait:.1f}s for interface response after retry...")
        await asyncio.sleep(retry_wait)
        
        # Check for file dialog
        if await check_for_file_dialog_async(page):
            print("[ASYNC_UPLOAD] 📁 File dialog opened after retry - success!")
            return True
        
        # Check for dropdown menu
        if await check_for_dropdown_menu_async(page):
            print("[ASYNC_UPLOAD] 📋 Menu appeared after retry - selecting post option")
            return await click_post_option_async(page)
        
        # Final broader detection attempt
        print("[ASYNC_UPLOAD] 🔄 Final broader detection attempt after retry...")
        final_result = await try_broader_upload_detection_async(page)
        
        if final_result:
            print("[ASYNC_UPLOAD] ✅ Success after retry with page refresh!")
            return True
        else:
            print("[ASYNC_UPLOAD] ❌ All retry attempts failed")
            return False
        
    except Exception as e:
        print(f"[ASYNC_UPLOAD] ❌ Error in retry with page refresh: {str(e)}")
        return False
...
async def check_for_file_dialog_async(page) -> bool:
    ...
    try:
        print("[ASYNC_UPLOAD] 🔍 Checking for file dialog...")
            ...
            try:
                ...
                for element in elements:
                    if await element.is_visible():
                        print(f"[ASYNC_UPLOAD] ✅ File dialog indicator found: {selector}")
                        return True
            except Exception as e:
                print(f"[ASYNC_UPLOAD] Selector failed: {selector} - {str(e)}")
                continue
        ...
        try:
            ...
            if 'create' in current_url.lower():
                print(f"[ASYNC_UPLOAD] 🔍 URL indicates create mode: {current_url}")
                return True
        except:
            pass
        ...
        try:
            ...
            for keyword in upload_keywords:
                if keyword in page_text.lower():
                    print(f"[ASYNC_UPLOAD] ✅ Upload interface detected via keyword: '{keyword}'")
                    return True
        except:
            pass
        
        print("[ASYNC_UPLOAD] ❌ No file dialog detected")
        return False
        
    except Exception as e:
        print(f"[ASYNC_UPLOAD] Error checking for file dialog: {str(e)}")
        return False
...
async def check_for_dropdown_menu_async(page) -> bool:
    ...
    try:
        ...
        if menu_element:
            print("[ASYNC_UPLOAD] ✅ Dropdown menu detected")
            return True
        ...
        for selector in SelectorConfig.POST_OPTION[:3]:
            try:
                ...
                if post_option and await post_option.is_visible():
                    print(f"[ASYNC_UPLOAD] ✅ Post option visible in menu")
                    return True
            except:
                continue
        ...
        print(f"[ASYNC_UPLOAD] Error checking for dropdown menu: {str(e)}")
        return False
...
async def try_broader_upload_detection_async(page) -> bool:
    ...
    try:
        print("[ASYNC_UPLOAD] 🔄 Attempting enhanced broader upload interface detection...")
        
        # ENHANCED: Additional wait before broader detection
        broader_wait = 3.0 + random.uniform(-1.0, 1.0)
        print(f"[ASYNC_UPLOAD] ⏳ Waiting {broader_wait:.1f}s before broader detection...")
        ...
        for indicator in upload_indicators:
            try:
                element = await page.query_selector(indicator)
                if element and await element.is_visible():
                    print(f"[ASYNC_UPLOAD] 🎯 Found upload indicator: {indicator}")
                    ...
                    if any(keyword in indicator.lower() for keyword in ['button', 'div[role="button"]']):
                        print(f"[ASYNC_UPLOAD] 🖱️ Clicking interactive element: {indicator}")
                        await click_element_with_behavior_async(page, element, "UPLOAD_INDICATOR")
                        
                        # Wait after clicking
                        click_wait = 2.0 + random.uniform(-0.5, 0.5)
                        print(f"[ASYNC_UPLOAD] ⏳ Waiting {click_wait:.1f}s after clicking indicator...")
                        await asyncio.sleep(click_wait)
                        
                        # Check if file dialog appeared after clicking
                        if await check_for_file_dialog_async(page):
                            print("[ASYNC_UPLOAD] ✅ File dialog appeared after clicking indicator!")
                            return True
                    
                    # For file inputs, just return True as they indicate upload interface
                    elif 'input' in indicator.lower():
                        print(f"[ASYNC_UPLOAD] ✅ File input found: {indicator}")
                        return True
                    
                    return True
            except Exception as e:
                print(f"[ASYNC_UPLOAD] Selector failed: {indicator} - {str(e)}")
                continue
        ...
        try:
            ...
            print(f"[ASYNC_UPLOAD] Error checking page content: {str(e)}")
        ...
        try:
            ...
            for indicator in url_indicators:
                if indicator in current_url:
                    print(f"[ASYNC_UPLOAD] ✅ Upload interface detected via URL: '{indicator}' in {current_url}")
                    return True
        except Exception as e:
            print(f"[ASYNC_UPLOAD] Error checking URL: {str(e)}")
        
        print("[ASYNC_UPLOAD] ⚠️ Could not detect upload interface with enhanced detection")
        return False
        
    except Exception as e:
        print(f"[ASYNC_UPLOAD] ❌ Error in enhanced broader detection: {str(e)}")
        return False
...
async def click_post_option_async(page) -> bool:
    ...
    try:
        print("[ASYNC_UPLOAD] 🔍 Looking for 'Публикация' option with enhanced detection...")
        ...
        print(f"[ASYNC_UPLOAD] 📖 Reading menu options for {reading_time:.1f}s (enhanced wait)...")
        ...
        for i, selector in enumerate(SelectorConfig.POST_OPTION):
            try:
                ...
                print(f"[ASYNC_UPLOAD] 🔍 Trying selector {i+1}/{len(SelectorConfig.POST_OPTION)}: {selector}")
                ...
                if post_option and await post_option.is_visible():
                    try:
                        ...
                        if any(keyword in combined_text for keyword in post_keywords):
                            print(f"[ASYNC_UPLOAD] ✅ Found 'Публикация' option: '{element_text.strip()}'")
                            found_selector = selector
                            break
                        else:
                            print(f"[ASYNC_UPLOAD] 🔍 Element found but text doesn't match: '{element_text.strip()}'")
                            post_option = None
                            continue
                    except Exception as text_error:
                        print(f"[ASYNC_UPLOAD] ✅ Found potential 'Публикация' option (text check failed: {str(text_error)})")
                        found_selector = selector
                        break
        ...
        if post_option:
            try:
                print(f"[ASYNC_UPLOAD] 🖱️ Clicking post option found with selector: {found_selector}")
                
                # ENHANCED: Pre-click wait
                pre_click_wait = 1.0 + random.uniform(-0.3, 0.3)
                print(f"[ASYNC_UPLOAD] ⏳ Waiting {pre_click_wait:.1f}s before clicking...")
                ...
                print(f"[ASYNC_UPLOAD] ⏳ Waiting {post_click_wait:.1f}s after clicking post option...")
                await asyncio.sleep(post_click_wait)
                
                # Check if file dialog appeared
                if await check_for_file_dialog_async(page):
                    print("[ASYNC_UPLOAD] ✅ File dialog appeared after clicking post option!")
                    return True
                ...
                try:
                    ...
                    if 'create' in current_url:
                        print(f"[ASYNC_UPLOAD] ✅ Successfully navigated to create page: {current_url}")
                        return True
                except:
                    pass
                
                # ENHANCED: Additional verification
                print("[ASYNC_UPLOAD] 🔍 Verifying post option click result...")
                verification_wait = 3.0 + random.uniform(-0.5, 0.5)
                await asyncio.sleep(verification_wait)
                
                if await check_for_file_dialog_async(page):
                    print("[ASYNC_UPLOAD] ✅ File dialog appeared after verification!")
                    return True
                
                print("[ASYNC_UPLOAD] ✅ Post option click completed (assuming success)")
                return True
                
            except Exception as click_error:
                print(f"[ASYNC_UPLOAD] ❌ Error clicking post option: {str(click_error)}")
                
                # ENHANCED: Fallback click attempt
                try:
                    print("[ASYNC_UPLOAD] 🔄 Trying fallback click method...")
                    await post_option.click(timeout=5000)
                    
                    fallback_wait = 3.0 + random.uniform(-0.5, 0.5)
                    print(f"[ASYNC_UPLOAD] ⏳ Waiting {fallback_wait:.1f}s after fallback click...")
                    await asyncio.sleep(fallback_wait)
                    
                    if await check_for_file_dialog_async(page):
                        print("[ASYNC_UPLOAD] ✅ Fallback click successful!")
                        return True
                    else:
                        print("[ASYNC_UPLOAD] ✅ Fallback click completed (assuming success)")
                        return True
                        
                except Exception as fallback_error:
                    print(f"[ASYNC_UPLOAD] ❌ Fallback click also failed: {str(fallback_error)}")
                    return False
        else:
            print("[ASYNC_UPLOAD] ❌ Post option not found with any selector")
            return False
        
    except Exception as e:
        print(f"[ASYNC_UPLOAD] ❌ Error in enhanced click_post_option: {str(e)}")
        return False
...
async def navigate_to_upload_alternative_async(page) -> bool:
    ...
    try:
        print("[ASYNC_UPLOAD] 🔄 Using alternative navigation: direct URL")
        ...
        if 'instagram.com' in current_url:
            ...
            if await check_for_file_dialog_async(page):
                print("[ASYNC_UPLOAD] ✅ Successfully navigated to upload page via direct URL")
                return True
            else:
                print("[ASYNC_UPLOAD] ⚠️ Direct URL navigation didn't show file dialog")
                return False
        else:
            print("[ASYNC_UPLOAD] ❌ Not on Instagram domain, cannot use direct URL")
            return False
            
    except Exception as e:
        print(f"[ASYNC_UPLOAD_ALT_ERROR] Alternative navigation failed: {str(e)}")
        return False
...
async def upload_video_with_human_behavior_async(page, video_file_path, video_obj):
    ...
    try:
        print(f"[ASYNC_UPLOAD] 🎬 Starting video upload following exact Selenium pipeline: {os.path.basename(video_file_path)}")
        ...
        print(f"[ASYNC_UPLOAD] ❌ Upload failed: {str(e)}")
        return False
...
async def upload_video_core_async(page, video_file_path, video_obj):
    ...
    try:
        print(f"[ASYNC_UPLOAD] 📁 Starting adaptive file input search for: {os.path.basename(video_file_path)}")
        
        # ENHANCED: Verify file exists before attempting upload
        if not os.path.exists(video_file_path):
            print(f"[ASYNC_UPLOAD] ❌ File does not exist: {video_file_path}")
            return False
        
        file_size = os.path.getsize(video_file_path)
        if file_size == 0:
            print(f"[ASYNC_UPLOAD] ❌ File is empty: {video_file_path}")
            return False
        
        print(f"[ASYNC_UPLOAD] ✅ File verified: {os.path.basename(video_file_path)} ({file_size} bytes)")
        
        # Find file input with adaptive search
        file_input = await find_file_input_adaptive_async(page)
        if not file_input:
            print("[ASYNC_UPLOAD] ❌ Failed to find file input")
            return False
        
        # Set file on input
        print(f"[ASYNC_UPLOAD] 📤 Setting file on input: {video_file_path}")
        await file_input.set_input_files(video_file_path)
        print("[ASYNC_UPLOAD] ✅ Video file selected successfully")
        
        # Minimal wait for processing (ПОЛНАЯ КОПИЯ из sync)
        processing_delay = random.uniform(2, 3)
        print(f"[ASYNC_UPLOAD] ⏳ Waiting {processing_delay:.1f}s for file processing...")
        ...
        try:
            print(f"[ASYNC_UPLOAD] 🔍 Extracting caption from video_obj type: {type(video_obj)}")
            ...
            if hasattr(video_obj, 'title') and video_obj.title:
                ...
                print(f"[ASYNC_UPLOAD] 📝 Found caption from VideoData.title: '{caption_text[:50]}...'")
            elif hasattr(video_obj, 'description') and video_obj.description:
                caption_text = str(video_obj.description).strip()
                print(f"[ASYNC_UPLOAD] 📝 Found caption from VideoData.description: '{caption_text[:50]}...'")
            ...
            # Strategy 2: Check for VideoTitle objects (from title_data relationship)
                ...
                if hasattr(video_obj.title_data, 'title') and video_obj.title_data.title:
                    ...
                    print(f"[ASYNC_UPLOAD] 📝 Found caption from VideoTitle.title: '{caption_text[:50]}...'")
                else:
                    print(f"[ASYNC_UPLOAD] ⚠️ VideoTitle object has no title attribute")
            
            # Strategy 3: Check for Django model objects (old async version)
            elif hasattr(video_obj, 'title_data') and video_obj.title_data:
                if hasattr(video_obj.title_data, 'title') and video_obj.title_data.title:
                    caption_text = str(video_obj.title_data.title).strip()
                    print(f"[ASYNC_UPLOAD] 📝 Found caption from Django model title_data.title: '{caption_text[:50]}...'")
                    ...
                    print(f"[ASYNC_UPLOAD] 📝 Found caption from Django model title_data.description: '{caption_text[:50]}...'")
            ...
            # Strategy 4: Check for direct attributes on video_obj
                ...
                print(f"[ASYNC_UPLOAD] 📝 Found caption from video_obj.caption: '{caption_text[:50]}...'")
            ...
            # Strategy 5: Check for any text-like attributes
                ...
                print(f"[ASYNC_UPLOAD] 🔍 All video_obj attributes: {all_attrs}")
                ...
                for attr in text_attrs:
                    if hasattr(video_obj, attr):
                        ...
                        if value and str(value).strip():
                            ...
                            print(f"[ASYNC_UPLOAD] 📝 Found caption from video_obj.{attr}: '{caption_text[:50]}...'")
                            break
                
                if not caption_text:
                    print(f"[ASYNC_UPLOAD] ⚠️ No caption text found in any attribute")
                    
        except Exception as caption_error:
            print(f"[ASYNC_UPLOAD] ❌ Error extracting caption: {str(caption_error)}")
            import traceback
            print(f"[ASYNC_UPLOAD] 🔍 Caption extraction traceback: {traceback.format_exc()}")
            caption_text = None
        
        # ENHANCED: Add caption if we found any text
        if caption_text and caption_text.strip():
            print(f"[ASYNC_UPLOAD] 🎯 Adding caption: '{caption_text[:100]}...'")
            caption_success = await add_video_caption_async(page, caption_text)
            if not caption_success:
                print(f"[ASYNC_UPLOAD] ⚠️ Failed to add caption, but continuing...")
        else:
            print(f"[ASYNC_UPLOAD] ⚠️ No caption text to add")
        
        # Add location if available (like sync _set_location_selenium_style)
        print(f"[ASYNC_UPLOAD] 📍 Adding location for video_obj: {type(video_obj)}")
        await add_video_location_async(page, video_obj)
        
        # Add mentions if available (like sync _set_mentions_selenium_style)
        print(f"[ASYNC_UPLOAD] 👥 Adding mentions for video_obj: {type(video_obj)}")
        await add_video_mentions_async(page, video_obj)
        
        # Click Share button (like sync _post_video_selenium_style)
        print("[ASYNC_UPLOAD] 🚀 Clicking Share button...")
        share_success = await click_share_button_async(page)
        if not share_success:
            print("[ASYNC_UPLOAD] ❌ Failed to click Share button")
            return False
        
        # Verify upload success (like sync _verify_video_posted)
        print("[ASYNC_UPLOAD] ✅ Verifying upload success...")
        return await check_video_posted_successfully_async(page, video_file_path)
        
    except Exception as e:
        print(f"[ASYNC_UPLOAD] ❌ Core upload failed: {str(e)}")
        import traceback
        print(f"[ASYNC_UPLOAD] 🔍 Full traceback: {traceback.format_exc()}")
        return False

async def find_file_input_adaptive_async(page):
    """Адаптивный поиск файлового input - ПОЛНАЯ КОПИЯ sync версии"""
    try:
        print("[ASYNC_UPLOAD] 🔍 Starting adaptive file input search...")
        
        # 🎯 СТРАТЕГИЯ 1: Поиск по семантическим атрибутам (ПОЛНАЯ КОПИЯ из sync)
        print("[ASYNC_UPLOAD] 📋 Strategy 1: Semantic attributes search...")
        semantic_strategies = [
            'input[type="file"]',
            'input[accept*="video"]',
            'input[accept*="image"]', 
        ...
        for selector in semantic_strategies:
            try:
                elements = await page.query_selector_all(selector)
                print(f"[ASYNC_UPLOAD] 🔎 Checking selector: {selector} - found {len(elements)} elements")
                
                for element in elements:
                    if element and await is_valid_file_input_async(element):
                        print(f"[ASYNC_UPLOAD] ✅ Found file input via semantic: {selector}")
                        return element
            except Exception as e:
                print(f"[ASYNC_UPLOAD] Semantic selector failed: {selector} - {str(e)}")
                continue
        
        # 🎯 СТРАТЕГИЯ 2: Поиск через структуру диалога (ПОЛНАЯ КОПИЯ из sync)
        print("[ASYNC_UPLOAD] 🏗️ Strategy 2: Dialog structure search...")
        dialog_input = await find_input_via_dialog_structure_async(page)
        if dialog_input:
            print("[ASYNC_UPLOAD] ✅ Found file input via dialog structure")
            return dialog_input
        
        # 🎯 СТРАТЕГИЯ 3: Поиск через кнопку "Выбрать на компьютере" (ПОЛНАЯ КОПИЯ из sync)
        print("[ASYNC_UPLOAD] 🔘 Strategy 3: Button-based search...")
        button_input = await find_input_via_button_async(page)
        if button_input:
            print("[ASYNC_UPLOAD] ✅ Found file input via button search")
            return button_input
        
        # 🎯 СТРАТЕГИЯ 4: Поиск по контексту формы (ПОЛНАЯ КОПИЯ из sync)
        print("[ASYNC_UPLOAD] 📝 Strategy 4: Form context search...")
        form_input = await find_input_via_form_context_async(page)
        if form_input:
            print("[ASYNC_UPLOAD] ✅ Found file input via form context")
            return form_input
        
        # 🎯 СТРАТЕГИЯ 5: Поиск по характерным CSS-классам Instagram (ПОЛНАЯ КОПИЯ из sync)
        print("[ASYNC_UPLOAD] 🎨 Strategy 5: Instagram CSS patterns...")
        css_input = await find_input_via_css_patterns_async(page)
        if css_input:
            print("[ASYNC_UPLOAD] ✅ Found file input via CSS patterns")
            return css_input
        
        # 🎯 СТРАТЕГИЯ 6: Широкий поиск всех input и фильтрация (ПОЛНАЯ КОПИЯ из sync)
        print("[ASYNC_UPLOAD] 🌐 Strategy 6: Broad search with filtering...")
        all_input = await find_input_via_broad_search_async(page)
        if all_input:
            print("[ASYNC_UPLOAD] ✅ Found file input via broad search")
            return all_input
            
        print("[ASYNC_UPLOAD] ⚠️ No file input found with any adaptive strategy")
        return None
        
    except Exception as e:
        print(f"[ASYNC_UPLOAD] ❌ Adaptive file input search failed: {str(e)}")
        return None
...
async def is_valid_file_input_async(element):
    ...
    try:
        ...
        print(f"[ASYNC_UPLOAD] Validating input: type='{input_type}', accept='{accept_attr}'")
        ...
        if accept_attr and not any(file_type in accept_attr.lower() for file_type in valid_types):
            print(f"[ASYNC_UPLOAD] Input rejected: accept attribute doesn't contain valid file types")
            return False
        
        # Если accept пустой, но это input[type="file"], считаем валидным (ПОЛНАЯ КОПИЯ из sync)
        if not accept_attr:
            print("[ASYNC_UPLOAD] Input has no accept attribute, but type='file' - considering valid")
        ...
        if multiple_attr is not None:
            print("[ASYNC_UPLOAD] Input supports multiple files - good sign")
        
        print("[ASYNC_UPLOAD] Input validation passed")
        return True
            
    except Exception as e:
        print(f"[ASYNC_UPLOAD] Error validating file input: {str(e)}")
        return False

async def find_input_via_dialog_structure_async(page):
    """Найти input через структуру диалога - ПОЛНАЯ КОПИЯ sync версии"""
    try:
        print("[ASYNC_UPLOAD] 🏗️ Searching within dialog structure...")
        ...
        for selector in dialog_selectors:
            try:
                ...
                if dialog:
                    print(f"[ASYNC_UPLOAD] 🏗️ Found dialog with: {selector}")
                    ...
                    if file_input and await is_valid_file_input_async(file_input):
                        print("[ASYNC_UPLOAD] ✅ Found valid file input inside dialog")
                        return file_input
                    ...
                    if form:
                        ...
                        if form_input and await is_valid_file_input_async(form_input):
                            print("[ASYNC_UPLOAD] ✅ Found valid file input inside form within dialog")
                            return form_input
                            
            except Exception as e:
                print(f"[ASYNC_UPLOAD] Dialog selector failed {selector}: {str(e)}")
                continue
                
        return None
    except Exception as e:
        print(f"[ASYNC_UPLOAD] Dialog structure search failed: {str(e)}")
        return None
    ...
    try:
        ...
        for selector in button_selectors:
            try:
                ...
                print(f"[ASYNC_UPLOAD] Button search failed for {selector}: {str(e)}")
                continue
        return None
    except Exception as e:
        print(f"[ASYNC_UPLOAD] Button-based search failed: {str(e)}")
        return None
...
async def find_input_via_form_context_async(page):
    ...
    try:
        ...
        print(f"[ASYNC_UPLOAD] Form context search failed: {str(e)}")
        return None
    ...
    try:
        ...
        for pattern in css_patterns:
            try:
                ...
                print(f"[ASYNC_UPLOAD] 🎨 CSS pattern: {pattern} - found {len(elements)} elements")
                
                for element in elements:
                    if element and await is_valid_file_input_async(element):
                        print(f"[ASYNC_UPLOAD] ✅ Valid file input found with CSS pattern: {pattern}")
                        return element
                        
            except Exception as e:
                print(f"[ASYNC_UPLOAD] CSS pattern failed: {pattern} - {str(e)}")
                continue
                
        return None
    except Exception as e:
        print(f"[ASYNC_UPLOAD] CSS patterns search failed: {str(e)}")
        return None
            ...
            try:
                ...
                if await is_valid_file_input_async(input_element):
                    ...
                    if any(file_type in input_accept.lower() for file_type in ['video', 'image', 'mp4', 'jpeg', 'png']):
                        print(f"[ASYNC_UPLOAD] Found valid file input: accept='{input_accept}', classes='{input_classes[:50]}'")
                        return input_element
                        
            except Exception as e:
                print(f"[ASYNC_UPLOAD] Error checking input element: {str(e)}")
                continue
                
        return None
    except Exception as e:
        print(f"[ASYNC_UPLOAD] Broad search failed: {str(e)}")
        return None

async def click_next_button_async(page, step_number):
    """Click next button with human-like behavior - ПОЛНАЯ КОПИЯ sync версии"""
    try:
        print(f"[ASYNC_UPLOAD] Clicking next button for step {step_number}...")
            ...
            try:
                ...
                if next_button and await next_button.is_visible():
                    ...
                    if any(keyword in button_text.lower() for keyword in ['далее', 'next', 'продолжить', 'continue']):
                        print(f"[ASYNC_UPLOAD] 🎯 Found next button: '{button_text.strip()}'")
                        break
        ...
        if next_button:
            ...
            try:
                ...
                print(f"[ASYNC_UPLOAD] ✅ Successfully clicked next button for step {step_number}")
                return True
                
            except Exception as click_error:
                print(f"[ASYNC_UPLOAD] ⚠️ Error clicking next button: {str(click_error)}")
                
                # Fallback: try direct click
                try:
                    await next_button.click()
                    await asyncio.sleep(random.uniform(4, 6))
                    print(f"[ASYNC_UPLOAD] ✅ Successfully clicked next button (fallback) for step {step_number}")
                    return True
                except Exception as fallback_error:
                    print(f"[ASYNC_UPLOAD] ❌ Fallback click also failed: {str(fallback_error)}")
                    return False
        else:
            print(f"[ASYNC_UPLOAD] ⚠️ Next button not found for step {step_number}")
            return False
            
    except Exception as e:
        print(f"[ASYNC_UPLOAD] ❌ Error in next button click: {str(e)}")
        return False
...
async def add_video_caption_async(page, caption_text):
    ...
    try:
        if not caption_text:
            print("[ASYNC_UPLOAD] No caption text provided")
            return True
        
        print(f"[ASYNC_UPLOAD] Setting caption: {caption_text[:50]}...")
        ...
        for selector in caption_field_selectors:
            try:
                ...
                if element and await element.is_visible():
                    print(f"[ASYNC_UPLOAD] ✅ Found caption field: {selector}")
                    caption_field = element
                    break
            except Exception as e:
                print(f"[ASYNC_UPLOAD] Selector {selector} failed: {str(e)}")
                continue
        
        if not caption_field:
            print("[ASYNC_UPLOAD] ⚠️ Caption field not found")
            return False
        ...
        print("[ASYNC_UPLOAD] Pressing Enter for line break...")
        await caption_field.press('Enter')
        await asyncio.sleep(random.uniform(0.5, 1.0))
        
        print("[ASYNC_UPLOAD] ✅ Caption set successfully")
        return True
        
    except Exception as e:
        print(f"[ASYNC_UPLOAD] ❌ Error setting caption: {str(e)}")
        return False
...
async def _type_like_human_async(page, element, text):
    ...
    try:
        print("[ASYNC_UPLOAD] 🤖 Starting human-like typing...")
        ...
        while i < len(text):
            ...
            if random.random() < 0.02:  # 2% chance
                ...
                print(f"[ASYNC_UPLOAD] 🤔 Thinking {thinking_pause:.1f}s...")
        ...
        print("[ASYNC_UPLOAD] ✅ Human-like typing completed")
        
    except Exception as e:
        print(f"[ASYNC_UPLOAD] ❌ Error in human-like typing: {str(e)}")
...
async def add_video_location_async(page, video_obj):
    ...
    try:
        print(f"[ASYNC_UPLOAD] 🔍 Starting location search for video_obj: {type(video_obj)}")
        ...
        try:
            ...
            if hasattr(video_obj, 'location') and video_obj.location:
                ...
                print(f"[ASYNC_UPLOAD] Found VideoData location: {location}")
            # Check for Django model objects (old async version)
            elif hasattr(video_obj, 'location') and video_obj.location:
                location = video_obj.location.strip()
                print(f"[ASYNC_UPLOAD] Found Django model location: {location}")
                ...
                if hasattr(video_obj.bulk_task, 'default_location') and video_obj.bulk_task.default_location:
                    ...
                    print(f"[ASYNC_UPLOAD] Found Django model default_location: {location}")
        except Exception as location_error:
            print(f"[ASYNC_UPLOAD] ❌ Error accessing location attributes: {str(location_error)}")
            print(f"[ASYNC_UPLOAD] 🔍 Video object type: {type(video_obj)}")
            location = None
        
        # Debug: show all attributes of video_obj
        if not location:
            print(f"[ASYNC_UPLOAD] 🔍 Video object attributes: {[attr for attr in dir(video_obj) if not attr.startswith('_')]}")
            print(f"[ASYNC_UPLOAD] 🔍 Video object location attribute: {getattr(video_obj, 'location', 'NOT_FOUND')}")
        
        if not location:
            print("[ASYNC_UPLOAD] No location data")
            return True
        
        print(f"[ASYNC_UPLOAD] Setting location: {location}")
        ...
        for selector in location_field_selectors:
            try:
                ...
                if element and await element.is_visible():
                    print(f"[ASYNC_UPLOAD] ✅ Found location field: {selector}")
        ...
        if not location_field:
            print("[ASYNC_UPLOAD] ⚠️ Location field not found")
            return False
        ...
        for selector in suggestion_selectors:
            try:
                ...
                if element and await element.is_visible():
                    suggestion = element
                    print(f"[ASYNC_UPLOAD] ✅ Found location suggestion: {selector}")
                    break
            except Exception as e:
                print(f"[ASYNC_UPLOAD] Selector {selector} failed: {str(e)}")
                continue
        ...
        if suggestion:
            ...
            await asyncio.sleep(random.uniform(1.0, 2.0))
            print("[ASYNC_UPLOAD] ✅ Location set successfully")
        else:
            print("[ASYNC_UPLOAD] ⚠️ Location suggestions not found")
        ...
        print(f"[ASYNC_UPLOAD] ❌ Error setting location: {str(e)}")
        return False

async def add_video_mentions_async(page, video_obj):
    """Add video mentions - FULL VERSION like sync"""
    try:
        print(f"[ASYNC_UPLOAD] 🔍 Starting mentions search for video_obj: {type(video_obj)}")
        ...
        try:
            ...
            if hasattr(video_obj, 'mentions') and video_obj.mentions:
                ...
                print(f"[ASYNC_UPLOAD] Found VideoData mentions: {mentions}")
            # Check for Django model objects (old async version)
            elif hasattr(video_obj, 'mentions') and video_obj.mentions:
                mentions = video_obj.mentions.strip()
                print(f"[ASYNC_UPLOAD] Found Django model mentions: {mentions}")
            elif hasattr(video_obj, 'bulk_task') and video_obj.bulk_task:
                if hasattr(video_obj.bulk_task, 'default_mentions') and video_obj.bulk_task.default_mentions:
                    ...
                    print(f"[ASYNC_UPLOAD] Found Django model default_mentions: {mentions}")
        except Exception as mentions_error:
            print(f"[ASYNC_UPLOAD] ❌ Error accessing mentions attributes: {str(mentions_error)}")
            print(f"[ASYNC_UPLOAD] 🔍 Video object type: {type(video_obj)}")
            mentions = None
        ...
        if not mentions:
            print(f"[ASYNC_UPLOAD] 🔍 Video object attributes: {[attr for attr in dir(video_obj) if not attr.startswith('_')]}")
            print(f"[ASYNC_UPLOAD] 🔍 Video object mentions attribute: {getattr(video_obj, 'mentions', 'NOT_FOUND')}")
        
        if not mentions:
            print("[ASYNC_UPLOAD] No mentions data")
            return True
        
        # Split mentions into list
        mentions_list = [mention.strip() for mention in mentions.split(',') if mention.strip()]
        print(f"[ASYNC_UPLOAD] Setting mentions: {mentions_list}")
        ...
        for selector in mentions_field_selectors:
            try:
                ...
                if element and await element.is_visible():
                    print(f"[ASYNC_UPLOAD] ✅ Found mentions field: {selector}")
        ...
        if not mentions_field:
            print("[ASYNC_UPLOAD] ⚠️ Mentions field not found")
            return False
        
        # Add each mention one by one
        for mention in mentions_list:
            print(f"[ASYNC_UPLOAD] Adding mention: {mention}")
            ...
            for selector in mention_suggestion_selectors:
                try:
                    ...
                    if element and await element.is_visible():
                        ...
                        print(f"[ASYNC_UPLOAD] ✅ Found mention suggestion: {selector}")
                        break
                except Exception as e:
                    print(f"[ASYNC_UPLOAD] Selector {selector} failed: {str(e)}")
                    continue
            ...
            if mention_suggestion:
                ...
                try:
                    ...
                    if mention_text and mention.lower() in mention_text.lower():
                        print(f"[ASYNC_UPLOAD] ✅ Verified mention text: '{mention_text}' contains '{mention}'")
                    else:
                        print(f"[ASYNC_UPLOAD] ⚠️ Mention text mismatch: expected '{mention}', got '{mention_text}'")
                        ...
                        for fallback_selector in [
                            ...
                            try:
                                ...
                                if element and await element.is_visible():
                                    mention_suggestion = element
                                    print(f"[ASYNC_UPLOAD] ✅ Found fallback mention suggestion: {fallback_selector}")
                                    break
                            except Exception:
                                continue
                except Exception as verify_error:
                    print(f"[ASYNC_UPLOAD] ⚠️ Error verifying mention text: {str(verify_error)}")
                ...
                if mention_suggestion:
                    ...
                    await asyncio.sleep(random.uniform(1.0, 2.0))
                    print(f"[ASYNC_UPLOAD] ✅ Mention '{mention}' added successfully")
                else:
                    print(f"[ASYNC_UPLOAD] ⚠️ Could not verify correct mention for '{mention}'")
                    # Press Enter to try to accept typed mention
                    await mentions_field.press('Enter')
                    await asyncio.sleep(random.uniform(1.0, 1.5))
            else:
                print(f"[ASYNC_UPLOAD] ⚠️ Mention suggestion for '{mention}' not found")
                print(f"[ASYNC_UPLOAD] ℹ️ Continuing with publication - mention '{mention}' will be added as text")
                # Press Enter to try to accept typed mention
                await mentions_field.press('Enter')
                await asyncio.sleep(random.uniform(1.0, 1.5))
        
        # ENHANCED: Click "Done" button after all mentions (like sync version)
        print("[ASYNC_UPLOAD] 🔍 Looking for 'Done' button...")
        ...
        for selector in done_button_selectors:
            try:
                ...
                if element and await element.is_visible():
                    done_button = element
                    print(f"[ASYNC_UPLOAD] ✅ Found 'Done' button: {selector}")
                    break
            except Exception as e:
                print(f"[ASYNC_UPLOAD] Selector {selector} failed: {str(e)}")
                continue
        ...
        if done_button:
            ...
            await asyncio.sleep(random.uniform(2.0, 3.0))
            print("[ASYNC_UPLOAD] ✅ 'Done' button clicked successfully")
        else:
            print("[ASYNC_UPLOAD] ⚠️ 'Done' button not found, trying to press Enter")
            # Fallback: try to press Enter on mentions field
            await mentions_field.press('Enter')
            await asyncio.sleep(random.uniform(1.0, 2.0))
        
        # КРИТИЧЕСКОЕ ДОБАВЛЕНИЕ: Ожидание после mention для стабилизации страницы
        print("[ASYNC_UPLOAD] ⏳ Waiting for page to stabilize after mentions...")
        ...
        try:
            ...
            for indicator in upload_context_indicators:
                try:
                    ...
                    if element and await element.is_visible():
                        context_found = True
                        print(f"[ASYNC_UPLOAD] ✅ Upload context confirmed: {indicator}")
                        break
                except Exception:
                    continue
            
            if not context_found:
                print("[ASYNC_UPLOAD] ⚠️ Upload context lost after mentions, trying to restore...")
                # Попытка восстановить контекст - нажать Tab для перехода к следующему элементу
                await page.keyboard.press('Tab')
                await asyncio.sleep(random.uniform(1.0, 2.0))
                
        except Exception as context_error:
            print(f"[ASYNC_UPLOAD] ⚠️ Error checking upload context: {str(context_error)}")
        
        print("[ASYNC_UPLOAD] ✅ All mentions processed")
        return True
        
    except Exception as e:
        print(f"[ASYNC_UPLOAD] ❌ Error setting mentions: {str(e)}")
        return False

async def click_share_button_async(page):
    """Click share button - ENHANCED VERSION with multiple selectors and context checking"""
    try:
        print("[ASYNC_UPLOAD] Looking for share button...")
        
        # КРИТИЧЕСКОЕ ДОБАВЛЕНИЕ: Проверяем контекст загрузки перед поиском кнопки
        print("[ASYNC_UPLOAD] 🔍 Checking upload context before searching for share button...")
        upload_context_indicators = [
            'input[placeholder="Добавить соавторов"]',
            'textarea[placeholder*="Напишите подпись"]',
            'input[placeholder*="Добавить местоположение"]',
        ...
        for indicator in upload_context_indicators:
            try:
                ...
                if element and await element.is_visible():
                    context_found = True
                    print(f"[ASYNC_UPLOAD] ✅ Upload context found: {indicator}")
                    break
            except Exception:
                continue
        
        if not context_found:
            print("[ASYNC_UPLOAD] ⚠️ Upload context not found, trying to restore...")
            ...
            try:
                ...
                if element and await element.is_visible():
                    ...
                    try:
                        ...
                        if parent_dialog:
                            print(f"[ASYNC_UPLOAD] ✅ Found share button in upload dialog: {selector}")
                            share_button = element
                            break
                        else:
                            print(f"[ASYNC_UPLOAD] ⚠️ Share button found but not in upload dialog: {selector}")
                            # Продолжаем поиск
                            continue
                    except Exception as context_error:
                        print(f"[ASYNC_UPLOAD] ⚠️ Error checking button context: {str(context_error)}")
                        # Если не можем проверить контекст, все равно используем кнопку
                        print(f"[ASYNC_UPLOAD] ✅ Found share button (context check failed): {selector}")
                        share_button = element
                        break
            except Exception as e:
                print(f"[ASYNC_UPLOAD] Selector {selector} failed: {str(e)}")
        ...
        if share_button:
            ...
            print("[ASYNC_UPLOAD] 🔍 Verifying share button before clicking...")
            ...
            try:
                ...
                print("[ASYNC_UPLOAD] ✅ Share button is visible and enabled")
            except Exception as verify_error:
                print(f"[ASYNC_UPLOAD] ⚠️ Share button verification failed: {str(verify_error)}")
            ...
            try:
                ...
                print("[ASYNC_UPLOAD] ✅ Share button clicked with JavaScript")
            except Exception as e:
                print(f"[ASYNC_UPLOAD] JavaScript click failed: {str(e)}")
                # Fallback to direct click
                await share_button.click()
                print("[ASYNC_UPLOAD] ✅ Share button clicked with direct click")
            
            # КРИТИЧЕСКОЕ ДОБАВЛЕНИЕ: Увеличиваем время ожидания для загрузки видео
            print("[ASYNC_UPLOAD] ⏳ Waiting for video to upload and process...")
            await asyncio.sleep(random.uniform(3, 5))  # Уменьшено с 8-12 до 3-5 секунд, поскольку проверка будет длительной
            return True
        else:
            print("[ASYNC_UPLOAD] ❌ Share button not found")
            ...
            try:
                ...
                print(f"[ASYNC_UPLOAD] 🔍 Available buttons on page ({len(all_buttons)} total):")
                for i, btn in enumerate(all_buttons[:15]):  # Show first 15 buttons
                    try:
                        ...
                        print(f"[ASYNC_UPLOAD] Button {i+1}: '{btn_text.strip()}' (aria: '{btn_aria}', class: '{btn_class[:50]}...')")
                    except Exception as debug_error:
                        print(f"[ASYNC_UPLOAD] Error debugging button {i+1}: {str(debug_error)}")
                
                # Показываем все div элементы с текстом
                all_divs = await page.query_selector_all('div')
                print(f"[ASYNC_UPLOAD] 🔍 Divs with text content:")
                for i, div in enumerate(all_divs[:20]):  # Show first 20 divs
                    try:
                        ...
                        if div_text.strip() and len(div_text.strip()) < 50:
                            ...
                            print(f"[ASYNC_UPLOAD] Div {i+1}: '{div_text.strip()}' (class: '{div_class[:50]}...')")
                    except Exception as debug_error:
                        continue
                        
            except Exception as e:
                print(f"[ASYNC_UPLOAD] Error listing elements: {str(e)}")
            
            return False
            
    except Exception as e:
        print(f"[ASYNC_UPLOAD] ❌ Error clicking share button: {str(e)}")
        return False
...
async def check_video_posted_successfully_async(page, video_file_path):
    ...
    try:
        print("[ASYNC_UPLOAD] Checking if video was posted successfully...")
        ...
        print(f"[ASYNC_UPLOAD] ⏳ Starting extended upload verification: {max_wait_time} seconds ({total_checks} checks every {check_interval}s)")
        ...
        for check_number in range(1, total_checks + 1):
            print(f"[ASYNC_UPLOAD] 🔍 Check {check_number}/{total_checks} - Looking for success/error indicators...")
            ...
            for selector in success_selectors:
                try:
                    ...
                    if success_element and await success_element.is_visible():
                        ...
                        print(f"[ASYNC_UPLOAD] ✅ SUCCESS DETECTED on check {check_number}: '{success_text.strip()}'")
                        print(f"[ASYNC_UPLOAD] ✅ Video {os.path.basename(video_file_path)} posted successfully")
                        return True
                except:
                    continue
            ...
            for selector in error_selectors:
                try:
                    ...
                    if error_element and await error_element.is_visible():
                        ...
                        print(f"[ASYNC_UPLOAD] ❌ ERROR DETECTED on check {check_number}: '{error_text.strip()}'")
                        print(f"[ASYNC_UPLOAD] ❌ Video {os.path.basename(video_file_path)} failed to post")
                        return False
                except:
                    continue
            ...
            if check_number < total_checks:
                print(f"[ASYNC_UPLOAD] ⏳ No indicators found on check {check_number}, waiting {check_interval}s before next check...")
                await asyncio.sleep(check_interval)
            else:
                print(f"[ASYNC_UPLOAD] ⏳ Final check {check_number} completed, no indicators found")
        
        # STRICT POLICY: If no explicit success indicators found after all checks, consider it a failure
        print(f"[ASYNC_UPLOAD] ❌ UPLOAD FAILED: No explicit success indicators found after {total_checks} checks ({max_wait_time}s total) - cannot confirm video {os.path.basename(video_file_path)} was posted")
        print(f"[ASYNC_UPLOAD] 🔍 Current page URL: {page.url}")
        
        return False
                
    except Exception as e:
        print(f"[ASYNC_UPLOAD] Error checking if video was posted: {str(e)}")
        return False
...
async def handle_cookie_consent_async(page):
    ...
    try:
        print("🍪 [ASYNC_COOKIES] Checking for cookie consent modal...")
        ...
        for i, indicator in enumerate(cookie_modal_indicators):
            try:
                ...
                if element and await element.is_visible():
                    modal_detected = True
                    print(f"🍪 [ASYNC_COOKIES] Cookie modal detected with indicator {i+1}")
                    break
        ...
        if not modal_detected:
            print("🍪 [ASYNC_COOKIES] No cookie consent modal found")
            return False
        ...
        print("🍪 [ASYNC_COOKIES] Attempting to accept all cookies...")
        ...
        for i, selector in enumerate(cookie_consent_buttons):
            try:
                print(f"🍪 [ASYNC_COOKIES] Trying selector {i+1}/{len(cookie_consent_buttons)}: {selector[:50]}...")
                ...
                if element and await element.is_visible():
                    ...
                    print(f"🍪 [ASYNC_COOKIES] Found accept button: '{button_text}' with selector: {selector}")
                    ...
                    # Click the button
                    await element.click()
                    print(f"✅ [ASYNC_COOKIES] Successfully clicked accept button: '{button_text}'")
                    ...
                    try:
                        ...
                        if not modal_still_present:
                            print("✅ [ASYNC_COOKIES] Cookie consent modal successfully dismissed")
                            return True
                        else:
                            print("⚠️ [ASYNC_COOKIES] Modal still present, trying next selector...")
                            continue
                            
                    except:
                        # If we can't verify, assume success
                        print("✅ [ASYNC_COOKIES] Cookie consent handling completed (verification skipped)")
                        return True
                    
            except Exception as e:
                print(f"❌ [ASYNC_COOKIES] Error with selector {i+1}: {str(e)}")
                continue
        
        # If no button worked, try clicking outside the modal (ПОЛНАЯ КОПИЯ из sync)
        print("⚠️ [ASYNC_COOKIES] No accept button worked, trying to click outside modal...")
        try:
            await page.click('body', position={'x': 50, 'y': 50})
            await asyncio.sleep(2)
            print("🍪 [ASYNC_COOKIES] Clicked outside modal as fallback")
            return True
        except:
            pass
        
        print("❌ [ASYNC_COOKIES] Could not handle cookie consent modal")
        return False
        
    except Exception as e:
        print(f"❌ [ASYNC_COOKIES] Cookie consent error: {str(e)}")
        return False
...
async def cleanup_browser_session_async(page, dolphin_browser, dolphin_profile_id, dolphin):
    ...
    try:
        ...
        if dolphin_browser:
            try:
                await dolphin_browser.cleanup_async()
                print("[ASYNC_CLEANUP] ✅ Dolphin browser cleanup completed")
            except Exception as cleanup_error:
                print(f"[ASYNC_CLEANUP] ⚠️ Dolphin browser cleanup error: {str(cleanup_error)}")
        
        # Additional cleanup for page if still available
        if page and not dolphin_browser:
            try:
                await page.close()
                print("[ASYNC_CLEANUP] ✅ Page closed directly")
            except Exception as page_error:
                print(f"[ASYNC_CLEANUP] ⚠️ Direct page cleanup error: {str(page_error)}")
        ...
        if dolphin and dolphin_profile_id:
            try:
                ...
                print(f"[ASYNC_CLEANUP] ✅ Stopped Dolphin profile: {dolphin_profile_id}")
            except Exception as dolphin_error:
                print(f"[ASYNC_CLEANUP] ⚠️ Dolphin profile stop error: {str(dolphin_error)}")
        
        print("[ASYNC_CLEANUP] ✅ Comprehensive cleanup completed")
        
    except Exception as cleanup_error:
        print(f"[ASYNC_CLEANUP] ❌ Critical cleanup error: {str(cleanup_error)}")
        
        # Emergency cleanup - force close everything
        try:
            print("[ASYNC_CLEANUP] 🚨 Attempting emergency cleanup...")
            ...
            print("[ASYNC_CLEANUP] 🚨 Emergency cleanup completed")
            
        except Exception as emergency_error:
            print(f"[ASYNC_CLEANUP] 💥 Emergency cleanup also failed: {str(emergency_error)}")
...
async def safely_close_all_windows_async(page, dolphin_browser, dolphin_profile_id=None):
    ...
    try:
        print("🔒 [ASYNC_BROWSER] Starting window closure...")
        
        if dolphin_browser:
            try:
                await dolphin_browser.cleanup_async()
                print("✅ [ASYNC_BROWSER] Dolphin browser closed via cleanup")
            except Exception as e:
                print(f"[ASYNC_BROWSER] Dolphin browser cleanup error: {str(e)}")
        elif page:
            try:
                await page.close()
                print("✅ [ASYNC_BROWSER] Page closed directly")
            except Exception as e:
                print(f"[ASYNC_BROWSER] Direct page close error: {str(e)}")
...
async def perform_instagram_login_optimized_async(page, account_details):
    ...
    try:
        ...
        print(f"🔐 [ASYNC_LOGIN] Starting login process for account: {account_details['username']}")
        
        # Don't navigate to login page - use fields on main page
        current_url = page.url
        print(f"🌐 [ASYNC_LOGIN] Using login fields on current page: {current_url}")
        ...
        for selector in username_selectors:
            try:
                ...
                if username_field and await username_field.is_visible():
                    print(f"✅ [ASYNC_LOGIN] Found username field: {selector}")
                    break
            except:
                continue
        
        if not username_field:
            print("❌ [ASYNC_LOGIN] Username field not found")
            return False
        ...
        for selector in password_selectors:
            try:
                ...
                if password_field and await password_field.is_visible():
                    print(f"✅ [ASYNC_LOGIN] Found password field: {selector}")
                    break
            except:
                continue
        
        if not password_field:
            print("❌ [ASYNC_LOGIN] Password field not found")
            return False
        
        # Clear fields and enter credentials
        print("📝 [ASYNC_LOGIN] Filling credentials...")
        ...
        for selector in login_selectors:
            try:
                ...
                if login_button and await login_button.is_visible():
                    print(f"✅ [ASYNC_LOGIN] Found login button: '{await login_button.text_content()}'")
                    break
            except:
                continue
        
        if not login_button:
            print("❌ [ASYNC_LOGIN] Login button not found")
            return False
        
        print("🔐 [ASYNC_LOGIN] Clicking login button...")
        ...
        if '/accounts/login' not in current_url and 'instagram.com' in current_url:
            print("✅ [ASYNC_LOGIN] Login successful")
            return True
        else:
            print("❌ [ASYNC_LOGIN] Login failed")
            return False
            
    except Exception as e:
        print(f"❌ [ASYNC_LOGIN] Enhanced login error: {str(e)}")
        return False
    ...
    try:
        ...
        for selector in phone_verification_indicators:
            try:
                ...
                if element and await element.is_visible():
                    element_text = await element.text_content() or ""
                    print(f"📱 [ASYNC_VERIFICATION] Phone verification detected: {element_text[:50]}")
                    return {
        ...
        print(f"❌ [ASYNC_VERIFICATION] Error checking phone verification: {str(e)}")
        return {'requires_phone_verification': False}
    ...
    try:
        ...
        for selector in human_verification_indicators:
            try:
                ...
                if element and await element.is_visible():
                    element_text = await element.text_content() or ""
                    print(f"🤖 [ASYNC_VERIFICATION] Human verification detected: {element_text[:50]}")
                    return {
        ...
        print(f"❌ [ASYNC_VERIFICATION] Error checking human verification: {str(e)}")
        return {'requires_human_verification': False}
    ...
    try:
        ...
        for selector in suspension_indicators:
            try:
                ...
                if element and await element.is_visible():
                    element_text = await element.text_content() or ""
                    print(f"🚫 [ASYNC_SUSPENSION] Account suspension detected: {element_text[:50]}")
                    return {
        ...
        print(f"❌ [ASYNC_SUSPENSION] Error checking account suspension: {str(e)}")
        return {'account_suspended': False}

async def handle_crop_async(page):
    """Handle crop interface - FULL ADAPTIVE VERSION like sync"""
    try:
        print("[ASYNC_CROP] 🖼️ Starting ADAPTIVE crop handling...")
            ...
            for selector in reels_dialog_selectors:
                ...
                if reels_dialog and await reels_dialog.is_visible():
                    ...
                    if dialog_text and any(keyword in dialog_text for keyword in ['Reels', 'видео', 'Теперь', 'Now']):
                        print("[ASYNC_CROP] ⚠️ Reels dialog still visible, trying to close it...")
                        await handle_reels_dialog_async(page)
                        await asyncio.sleep(random.uniform(1, 2))
                        break
        except:
        ...
        print(f"[ASYNC_CROP] ⏳ Waiting {initial_wait:.1f}s for crop page to load...")
        await asyncio.sleep(initial_wait)
        
        # First, verify if we're on a crop page using adaptive detection
        if not await _verify_crop_page_adaptive_async(page):
            print("[ASYNC_CROP] ℹ️ Not on crop page or crop not needed, skipping crop handling")
            return True
        
        # Use adaptive crop detection and handling
        if await _handle_crop_adaptive_async(page):
            print("[ASYNC_CROP] ✅ Crop handled successfully with adaptive method")
            return True
        else:
            print("[ASYNC_CROP] ⚠️ Adaptive crop handling failed, video may proceed with default crop")
            return True  # Don't fail the whole process
            
    except Exception as e:
        print(f"[ASYNC_CROP] ❌ Crop handling failed: {str(e)}")
...
async def _verify_crop_page_adaptive_async(page):
    ...
    try:
        print("[ASYNC_CROP] 🔍 Verifying crop page...")
        ...
        for selector in crop_indicators:
            try:
                element = await page.query_selector(selector)
                if element and await element.is_visible():
                    print(f"[ASYNC_CROP] ✅ Found crop indicator: {selector}")
                    return True
            except:
                continue
        
        print("[ASYNC_CROP] ⚠️ No crop indicators found")
        return False
        
    except Exception as e:
        print(f"[ASYNC_CROP] ❌ Crop page verification failed: {str(e)}")
        return False

async def _handle_crop_adaptive_async(page):
    """Handle crop with adaptive detection - async version"""
    try:
        print("[ASYNC_CROP] 📐 Starting adaptive crop handling...")
        ...
        for strategy_index, strategy in enumerate(search_strategies, 1):
            print(f"[ASYNC_CROP] 📐 Trying strategy {strategy_index}: {strategy.__name__}")
            
            try:
                crop_button = await strategy(page)
                if crop_button:
                    print(f"[ASYNC_CROP] ✅ Found crop button using strategy {strategy_index}")
                    ...
                    if await _select_original_aspect_ratio_async(page):
                        print("[ASYNC_CROP] ✅ Original aspect ratio selected successfully")
                        return True
                    else:
                        print("[ASYNC_CROP] ⚠️ Failed to select original aspect ratio")
                        return True  # Continue anyway
                    
            except Exception as e:
                print(f"[ASYNC_CROP] Strategy {strategy_index} failed: {str(e)}")
                continue
        
        print("[ASYNC_CROP] ❌ All strategies failed - crop button not found")
        return False
        
    except Exception as e:
        print(f"[ASYNC_CROP] ❌ Adaptive crop handling failed: {str(e)}")
        return False

async def _find_crop_by_semantic_attributes_async(page):
    """Поиск по семантическим атрибутам (самый устойчивый) - async version"""
    print("[ASYNC_CROP] 📐 [SEMANTIC] Searching by semantic attributes...")
    ...
    for selector in semantic_selectors:
        try:
            print(f"[ASYNC_CROP] 📐 [SEMANTIC] Trying: {selector}")
            
            # Прямой поиск элемента
            element = await page.query_selector(selector)
            if element and await element.is_visible():
                print(f"[ASYNC_CROP] 📐 [SEMANTIC] ✅ Found direct element: {selector}")
                return element
            ...
            for parent_selector in parent_selectors:
                ...
                if parent_element and await parent_element.is_visible():
                    print(f"[ASYNC_CROP] 📐 [SEMANTIC] ✅ Found parent element: {parent_selector}")
                    return parent_element
                
        except Exception as e:
            print(f"[ASYNC_CROP] 📐 [SEMANTIC] Selector {selector} failed: {str(e)}")
            continue
...
async def _find_crop_by_svg_content_async(page):
    ...
    print("[ASYNC_CROP] 📐 [SVG] Searching by SVG content analysis...")
    
    try:
        # Находим все SVG элементы на странице
        svg_elements = await page.query_selector_all('svg')
        print(f"[ASYNC_CROP] 📐 [SVG] Found {len(svg_elements)} SVG elements")
        ...
        for svg in svg_elements:
            try:
                ...
                if any(keyword in aria_label.lower() for keyword in ['crop', 'обрез', 'размер', 'выбрать']):
                    print(f"[ASYNC_CROP] 📐 [SVG] ✅ Found by aria-label: {aria_label}")
                ...
                for path in paths:
                    ...
                    if any(pattern in path_data for pattern in ['M10 20H4v-6', 'M20.999 2H14', 'L', 'H', 'V']):
                        print(f"[ASYNC_CROP] 📐 [SVG] ✅ Found by SVG path pattern")
                ...
                print(f"[ASYNC_CROP] 📐 [SVG] SVG analysis failed: {str(e)}")
                continue
                
    except Exception as e:
        print(f"[ASYNC_CROP] 📐 [SVG] SVG content analysis failed: {str(e)}")
...
async def _find_crop_by_context_analysis_async(page):
    ...
    print("[ASYNC_CROP] 📐 [CONTEXT] Searching by context analysis...")
    ...
    try:
        ...
        for selector in context_selectors:
            try:
                ...
                print(f"[ASYNC_CROP] 📐 [CONTEXT] Found {len(buttons)} buttons with selector: {selector}")
                ...
                for button in buttons:
                    ...
                    if bbox and 10 <= bbox['width'] <= 50 and 10 <= bbox['height'] <= 50:
                        ...
                        if svg_inside and await svg_inside.is_visible():
                            print(f"[ASYNC_CROP] 📐 [CONTEXT] ✅ Found candidate button by context")
                            return button
                            
            except Exception as e:
                print(f"[ASYNC_CROP] 📐 [CONTEXT] Context selector {selector} failed: {str(e)}")
                continue
                
    except Exception as e:
        print(f"[ASYNC_CROP] 📐 [CONTEXT] Context analysis failed: {str(e)}")
    
    return None

async def _find_crop_by_fallback_patterns_async(page):
    """Поиск по резервным паттернам (последний resort) - async version"""
    print("[ASYNC_CROP] 📐 [FALLBACK] Using fallback patterns...")
    ...
    try:
        ...
        for xpath in xpath_selectors:
            try:
                print(f"[ASYNC_CROP] 📐 [FALLBACK] Trying XPath: {xpath}")
                element = await page.query_selector(f'xpath={xpath}')
                if element and await element.is_visible():
                    print(f"[ASYNC_CROP] 📐 [FALLBACK] ✅ Found by XPath: {xpath}")
                    return element
                    
            except Exception as e:
                print(f"[ASYNC_CROP] 📐 [FALLBACK] XPath {xpath} failed: {str(e)}")
                continue
                
    except Exception as e:
        print(f"[ASYNC_CROP] 📐 [FALLBACK] Fallback patterns failed: {str(e)}")
...
async def _human_click_crop_button_async(page, crop_button):
    ...
    try:
        print("[ASYNC_CROP] 📐 [CLICK] Performing human-like click on crop button...")
        ...
        print("[ASYNC_CROP] 📐 [CLICK] ✅ Successfully clicked crop button")
        
    except Exception as e:
        print(f"[ASYNC_CROP] 📐 [CLICK] ❌ Failed to click crop button: {str(e)}")
        raise
...
async def _select_original_aspect_ratio_async(page):
    ...
    print("[ASYNC_CROP] 📐 Looking for 'Оригинал' (Original) aspect ratio option...")
    ...
    for strategy_index, strategy in enumerate(search_strategies, 1):
        print(f"[ASYNC_CROP] 📐 [ORIGINAL] Trying strategy {strategy_index}: {strategy.__name__}")
        
        try:
            original_element = await strategy(page)
            if original_element:
                print(f"[ASYNC_CROP] 📐 [ORIGINAL] ✅ Found 'Оригинал' using strategy {strategy_index}")
                ...
                print(f"[ASYNC_CROP] ⏳ Waiting {aspect_ratio_wait:.1f}s for 'Оригинал' aspect ratio to be applied...")
                await asyncio.sleep(aspect_ratio_wait)
                
                return True
                
        except Exception as e:
            print(f"[ASYNC_CROP] 📐 [ORIGINAL] Strategy {strategy_index} failed: {str(e)}")
            continue
    
    print("[ASYNC_CROP] 📐 [ORIGINAL] ⚠️ All strategies failed to find 'Оригинал' option")
    return False

async def _find_original_by_text_content_async(page):
    """Поиск 'Оригинал' по текстовому содержимому - async version"""
    print("[ASYNC_CROP] 📐 [TEXT] Searching for 'Оригинал' by text content...")
    ...
    for selector in text_selectors:
        try:
            print(f"[ASYNC_CROP] 📐 [TEXT] Trying: {selector}")
            ...
            if element and await element.is_visible():
                ...
                if selector.startswith('span:'):
                    ...
                    if parent_button and await parent_button.is_visible():
                        print(f"[ASYNC_CROP] 📐 [TEXT] ✅ Found 'Оригинал' parent button")
                        return parent_button
                
                print(f"[ASYNC_CROP] 📐 [TEXT] ✅ Found 'Оригинал' element: {selector}")
                return element
                
        except Exception as e:
            print(f"[ASYNC_CROP] 📐 [TEXT] Selector {selector} failed: {str(e)}")
            continue
...
async def _find_original_by_svg_icon_async(page):
    ...
    print("[ASYNC_CROP] 📐 [SVG] Searching for 'Оригинал' by SVG icon...")
        ...
        for selector in svg_selectors:
            try:
                ...
                if svg_element and await svg_element.is_visible():
                    print(f"[ASYNC_CROP] 📐 [SVG] ✅ Found SVG icon: {selector}")
                    
                    # Найти родительскую кнопку
                    parent_button = await svg_element.query_selector('xpath=ancestor::*[@role="button"][1] | xpath=ancestor::button[1]')
                    if parent_button and await parent_button.is_visible():
                        print("[ASYNC_CROP] 📐 [SVG] ✅ Found parent button for SVG")
                        return parent_button
                    
                    return svg_element
                    
            except Exception as e:
                print(f"[ASYNC_CROP] 📐 [SVG] SVG selector {selector} failed: {str(e)}")
                continue
                
    except Exception as e:
        print(f"[ASYNC_CROP] 📐 [SVG] SVG search failed: {str(e)}")
    
    return None

async def _find_original_by_first_position_async(page):
    """Поиск 'Оригинал' по позиции (обычно первый элемент) - async version"""
    print("[ASYNC_CROP] 📐 [POS] Searching for 'Оригинал' by position...")
    ...
    try:
        ...
        for selector in position_selectors:
            try:
                ...
                if buttons:
                    print(f"[ASYNC_CROP] 📐 [POS] Found {len(buttons)} buttons with selector: {selector}")
                        ...
                        try:
                            if await button.is_visible():
                                ...
                                if 'Оригинал' in button_text or 'Original' in button_text:
                                    print(f"[ASYNC_CROP] 📐 [POS] ✅ Found 'Оригинал' at position {i+1}: '{button_text.strip()}'")
                                    return button
                                
                                # Если первая кнопка - возможно это "Оригинал"
                                if i == 0:
                                    print(f"[ASYNC_CROP] 📐 [POS] ✅ Using first button as potential 'Оригинал': '{button_text.strip()}'")
                                    return button
                                    
                        except Exception as e:
                            print(f"[ASYNC_CROP] 📐 [POS] Button {i+1} check failed: {str(e)}")
                            continue
                            
            except Exception as e:
                print(f"[ASYNC_CROP] 📐 [POS] Position selector {selector} failed: {str(e)}")
                continue
                
    except Exception as e:
        print(f"[ASYNC_CROP] 📐 [POS] Position search failed: {str(e)}")
    
    return None

async def _find_any_available_option_async(page):
    """Поиск любой доступной опции кропа (fallback) - async version"""
    print("[ASYNC_CROP] 📐 [ANY] Searching for any available crop option...")
    ...
    try:
        ...
        for selector in fallback_selectors:
            try:
                ...
                if element and await element.is_visible():
                    element_text = await element.text_content() or ""
                    print(f"[ASYNC_CROP] 📐 [ANY] ✅ Found fallback option: '{element_text.strip()}' with selector: {selector}")
                    return element
                    
            except Exception as e:
                print(f"[ASYNC_CROP] 📐 [ANY] Fallback selector {selector} failed: {str(e)}")
                continue
                
    except Exception as e:
        print(f"[ASYNC_CROP] 📐 [ANY] Fallback search failed: {str(e)}")
...
async def _human_click_with_timeout_async(page, element, log_prefix="HUMAN_CLICK"):
    ...
    try:
        ...
        try:
            ...
            await element.click()
            await asyncio.sleep(random.uniform(0.5, 1.0))
            print(f"[{log_prefix}] ✅ Human click successful")
            ...
            page.set_default_timeout(original_timeout)
            print(f"[{log_prefix}] Human behavior failed: {str(e)[:100]}")
            
            # Fallback to quick click
            return await _quick_click_async(page, element, log_prefix)
            
    except Exception as e:
        print(f"[{log_prefix}] Error in human click: {str(e)[:100]}")
        return False
...
async def _quick_click_async(page, element, log_prefix="QUICK_CLICK"):
    ...
    try:
        ...
        print(f"[{log_prefix}] ✅ Quick click successful")
        return True
    except Exception as e:
        try:
            # Fallback to JavaScript click
            await page.evaluate('(element) => element.click()', element)
            print(f"[{log_prefix}] ✅ JavaScript click successful")
            return True
        except Exception as e2:
            # Last resort: standard click with short timeout
            try:
                await element.click(timeout=2000)
                print(f"[{log_prefix}] ✅ Standard click successful")
                return True
            except Exception as e3:
                print(f"[{log_prefix}] ⚠️ All click methods failed: {str(e3)[:100]}")
                return False
...
async def simulate_page_scan_async(page):
    ...
    try:
        ...
        print(f"[ASYNC_HUMAN] Page scan simulation error: {str(e)}")
...
async def add_human_delay_between_uploads_async(page, upload_index):
    ...
    try:
        ...
        if random.random() < 0.3:  # 30% chance to simulate browsing
            print(f"[ASYNC_HUMAN] Simulating browsing behavior before next upload...")
            await simulate_random_browsing_async(page)
            base_delay += random.uniform(5, 10)
        
        print(f"[ASYNC_HUMAN] Waiting {base_delay:.1f}s before next upload...")
        await asyncio.sleep(base_delay)
        
    except Exception as e:
        print(f"[ASYNC_HUMAN] Delay simulation error: {str(e)}")
...
async def simulate_random_browsing_async(page):
    ...
    try:
        ...
        print(f"[ASYNC_HUMAN] Random browsing simulation error: {str(e)}")

async def perform_final_cleanup_async(page, username):
    """Perform final cleanup after operations - async version"""
    try:
        print(f"[ASYNC_CLEANUP] Performing final cleanup for account: {username}")
        
        # Navigate to home page to clear any upload state
        try:
            await page.goto("https://www.instagram.com/", wait_until="domcontentloaded", timeout=30000)
            await asyncio.sleep(random.uniform(2, 4))
            print("[ASYNC_CLEANUP] ✅ Navigated to home page")
        except Exception as nav_error:
            print(f"[ASYNC_CLEANUP] Navigation error: {str(nav_error)}")
        
        # Clear any temporary state
        try:
            await page.evaluate("localStorage.clear(); sessionStorage.clear();")
            print("[ASYNC_CLEANUP] ✅ Cleared browser storage")
        except Exception as storage_error:
            print(f"[ASYNC_CLEANUP] Storage cleanup error: {str(storage_error)}")
        
    except Exception as e:
        print(f"[ASYNC_CLEANUP] Final cleanup error: {str(e)}")

async def log_video_info_async(video_index, total_videos, video_file_path, video_obj):
    """Log video information - async version"""
    try:
        print(f"[ASYNC_UPLOAD] 📹 Processing video {video_index}/{total_videos}: {os.path.basename(video_file_path)}")
        ...
        if title:
            ...
            print(f"[ASYNC_UPLOAD] 📝 Title: {title_preview}")
        
        # Log file size
        if os.path.exists(video_file_path):
            file_size = os.path.getsize(video_file_path)
            file_size_mb = file_size / (1024 * 1024)
            print(f"[ASYNC_UPLOAD] 📊 File size: {file_size_mb:.1f} MB")
            
    except Exception as e:
        print(f"[ASYNC_UPLOAD] Error logging video info: {str(e)}")
...
async def perform_enhanced_instagram_login_async(page, account_details):
    ...
    try:
        ...
        print(f"🔐 [ASYNC_LOGIN] Starting enhanced login process for account: {username}")
        ...
        print("⏳ [ASYNC_LOGIN] Waiting after filling credentials before form submission...")
            ...
            try:
                ...
                if login_button and await login_button.is_visible():
                    ...
                    if any(keyword in button_text.lower() for keyword in ['войти', 'log in', 'login', 'submit']):
                        print(f"✅ [ASYNC_LOGIN] Found login button: '{button_text.strip()}'")
                        break
            except:
                continue
        ...
        if 'suspend' in error_message.lower():
            print(f"🚫 [ASYNC_LOGIN] Account suspended during login: {error_message}")
            return "SUSPENDED"
        else:
            print(f"❌ [ASYNC_LOGIN] Enhanced login error: {error_message}")
            return False
    ...
    try:
        ...
        try:
            ...
            if any(keyword in page_text.lower() for keyword in suspension_keywords):
                print("🚫 [ASYNC_LOGIN] Account suspension detected in login response")
                return "SUSPENDED"
            ...
            for error_element in error_elements:
                if await error_element.is_visible():
                    error_text = await error_element.text_content() or ""
                    if error_text.strip():
                        print(f"❌ [ASYNC_LOGIN] Login error detected: {error_text}")
...
async def handle_2fa_async(page, account_details):
    ...
    try:
        ...
        if not tfa_secret:
            print("❌ [ASYNC_2FA] 2FA required but no secret provided")
            return False
        
        print("📱 [ASYNC_2FA] Handling 2FA authentication...")
        ...
        if '/accounts/login' not in current_url and 'challenge' not in current_url:
            print("✅ [ASYNC_2FA] 2FA authentication successful")
            return True
        else:
...
async def handle_save_login_info_dialog_async(page):
    ...
    try:
        print("[ASYNC_SAVE_LOGIN] Checking for 'Save login info' dialog...")
        await asyncio.sleep(random.uniform(2, 4))
        ...
        print(f"[ASYNC_SAVE_LOGIN] Save login dialog detected: {is_save_login_dialog}")
        
        if is_save_login_dialog:
            print("[ASYNC_SAVE_LOGIN] 💾 Save login info dialog found")
                ...
                try:
                    ...
                    if save_button and await save_button.is_visible():
                        ...
                        if any(keyword in button_text.lower() for keyword in ['сохранить', 'save']):
                            print(f"[ASYNC_SAVE_LOGIN] ✅ Found save button: '{button_text.strip()}'")
                            break
                except:
                    continue
            ...
            if save_button:
                try:
                    ...
                    await asyncio.sleep(random.uniform(2, 4))
                    print("[ASYNC_SAVE_LOGIN] ✅ Successfully clicked save login info button")
                    return True
                except Exception as e:
                    print(f"[ASYNC_SAVE_LOGIN] ❌ Error clicking save button: {str(e)}")
            
            # If no save button, look for "Not now" button
            print("[ASYNC_SAVE_LOGIN] ⚠️ Could not find save button, looking for 'Not now' button...")
            not_now_selectors = [
            ...
            for selector in not_now_selectors:
                try:
                    ...
                    if not_now_button and await not_now_button.is_visible():
                        ...
                        print(f"[ASYNC_SAVE_LOGIN] Found 'Not now' button: '{button_text.strip()}'")
                        
                        await not_now_button.hover()
                        await asyncio.sleep(random.uniform(0.5, 1.0))
                        await not_now_button.click()
                        await asyncio.sleep(random.uniform(2, 4))
                        print("[ASYNC_SAVE_LOGIN] ✅ Successfully clicked 'Not now' button")
                        return True
                except:
                    continue
        
        print("[ASYNC_SAVE_LOGIN] No save login dialog found or handled")
        return True
        
    except Exception as e:
        print(f"❌ [ASYNC_SAVE_LOGIN] Error handling save login dialog: {str(e)}")
        return True  # Continue anyway
...
async def run_bulk_upload_task_parallel_async(task_id):
    ...
    """
    try:
        print(f"🚀 [PARALLEL_ASYNC] Starting parallel bulk upload task {task_id}")
        
        # Get task and accounts
        task_data = await get_task_with_accounts_async(task_id)
        if not task_data:
            print(f"❌ [PARALLEL_ASYNC] Task {task_id} not found")
            return False
        ...
        if total_accounts == 0:
            print(f"⚠️ [PARALLEL_ASYNC] No accounts found for task {task_id}")
            return True
        
        print(f"📊 [PARALLEL_ASYNC] Processing {total_accounts} accounts with max {PARALLEL_CONFIG['MAX_CONCURRENT_ACCOUNTS']} concurrent")
        ...
        for batch_num, batch in enumerate(account_batches, 1):
            print(f"📦 [PARALLEL_ASYNC] Processing batch {batch_num}/{len(account_batches)} ({len(batch)} accounts)")
            ...
            if batch_num < len(account_batches):
                ...
                print(f"⏳ [PARALLEL_ASYNC] Waiting {delay:.1f}s before next batch...")
                await asyncio.sleep(delay)
        
        # Final summary
        print(f"✅ [PARALLEL_ASYNC] Task {task_id} completed:")
        print(f"   📈 Successful: {results['successful']}")
        print(f"   ❌ Failed: {results['failed']}")
        print(f"   📱 Phone verification: {results['phone_verification']}")
        print(f"   🤖 Human verification: {results['human_verification']}")
        ...
        try:
            ...
            if deleted_files > 0:
                print(f"🗑️ [PARALLEL_ASYNC] Cleaned up {deleted_files} original video files from media directory")
        except Exception as e:
            print(f"⚠️ [PARALLEL_ASYNC] Failed to cleanup original video files: {str(e)}")
        
        return True
        
    except Exception as e:
        print(f"💥 [PARALLEL_ASYNC] Critical error in parallel task {task_id}: {str(e)}")
        ...
        try:
            deleted_files = await cleanup_original_video_files_async(task_id)
            if deleted_files > 0:
                print(f"🗑️ [PARALLEL_ASYNC] Cleaned up {deleted_files} original video files after error")
        except Exception as cleanup_error:
            print(f"⚠️ [PARALLEL_ASYNC] Failed to cleanup original video files after error: {str(cleanup_error)}")
                ...
                try:
                    ...
                    if file_path and os.path.exists(file_path):
                        ...
                        if is_safe:
                            ...
                            deleted_count += 1
                            print(f"🗑️ [CLEANUP] Deleted original video file: {filename}")
                        else:
                            print(f"⏸️ [CLEANUP] Skipped deleting file (still in use by other tasks): {os.path.basename(file_path)}")
                            if warning_msg:
                                print(f"⚠️ [CLEANUP] {warning_msg}")
                        
                except Exception as e:
                    print(f"⚠️ [CLEANUP] Failed to delete video file {video.id}: {str(e)}")
        
        return deleted_count
        
    except Exception as e:
        print(f"❌ [CLEANUP] Error in cleanup process: {str(e)}")
        return 0
...
async def process_account_batch_async(account_tasks, task, batch_num):
    ...
    try:
        print(f"🔄 [BATCH_{batch_num}] Starting batch processing with {len(account_tasks)} accounts")
        ...
        print(f"⚡ [BATCH_{batch_num}] Executing {len(account_coroutines)} account tasks concurrently...")
        ...
        for i, result in enumerate(account_results):
            if isinstance(result, Exception):
                print(f"💥 [BATCH_{batch_num}] Account {i+1} failed with exception: {str(result)}")
        ...
        print(f"✅ [BATCH_{batch_num}] Completed: {batch_results['successful']} successful, {batch_results['failed']} failed")
        return batch_results
        
    except Exception as e:
        print(f"💥 [BATCH_{batch_num}] Batch processing error: {str(e)}")
    ...
    async with semaphore:
        try:
            ...
            if not account_details:
                print(f"❌ [BATCH_{batch_num}] Account {account_task.account_id} details not found")
                return ("ERROR", 0, 1)
            
            username = account_details['username']
            print(f"🎯 [BATCH_{batch_num}] Starting account: {username}")
            
            # Add random delay to avoid simultaneous starts
            start_delay = random.uniform(*PARALLEL_CONFIG['ACCOUNT_START_DELAY'])
            print(f"⏳ [BATCH_{batch_num}] Account {username} waiting {start_delay:.1f}s before start...")
            await asyncio.sleep(start_delay)
            
            # Get videos for this account
            videos = await get_assigned_videos_async(account_task.id)
            if not videos:
                print(f"⚠️ [BATCH_{batch_num}] No videos assigned to account {username}")
                return ("SUCCESS", 0, 0)
            
            # Prepare uniquified videos
            video_files = await prepare_unique_videos_async(account_task, videos)
            if not video_files:
                print(f"❌ [BATCH_{batch_num}] Failed to prepare videos for account {username}")
                return ("ERROR", 0, 1)
            
            # Process account with retries
            for attempt in range(1, PARALLEL_CONFIG['MAX_RETRIES_PER_ACCOUNT'] + 1):
                try:
                    print(f"🔄 [BATCH_{batch_num}] Account {username} attempt {attempt}/{PARALLEL_CONFIG['MAX_RETRIES_PER_ACCOUNT']}")
                    ...
                    if result[0] in ["SUCCESS", "SUSPENDED", "PHONE_VERIFICATION_REQUIRED", "HUMAN_VERIFICATION_REQUIRED"]:
                        print(f"✅ [BATCH_{batch_num}] Account {username} completed: {result[0]}")
                        return result
                    ...
                    if attempt < PARALLEL_CONFIG['MAX_RETRIES_PER_ACCOUNT']:
                        ...
                        print(f"⚠️ [BATCH_{batch_num}] Account {username} failed, retrying in {retry_delay:.1f}s...")
                        await asyncio.sleep(retry_delay)
                    
                except Exception as e:
                    print(f"💥 [BATCH_{batch_num}] Account {username} attempt {attempt} exception: {str(e)}")
                    if attempt >= PARALLEL_CONFIG['MAX_RETRIES_PER_ACCOUNT']:
                        return ("ERROR", 0, 1)
                    await asyncio.sleep(random.uniform(30, 60))
            
            # All retries exhausted
            print(f"❌ [BATCH_{batch_num}] Account {username} failed after all retries")
            return ("ERROR", 0, 1)
            
        except Exception as e:
            print(f"💥 [BATCH_{batch_num}] Critical error processing account: {str(e)}")
            return ("ERROR", 0, 1)
...
async def get_task_with_accounts_async(task_id):
    ...
    try:
        ...
        print(f"❌ [DATABASE] Error getting task data: {str(e)}")
        return None
...
async def get_account_details_async(account_id):
    ...
    try:
        ...
        print(f"❌ [DATABASE] Error getting account details: {str(e)}")
        return None
    ...
    try:
        ...
        def get_videos():
            try:
                ...
                print(f"[ASYNC_DATA] Loaded {len(videos)} videos with {len(all_titles)} titles")
                
                # CRITICAL FIX: Assign titles to videos if not already assigned (like sync version)
                if all_titles:
                    print(f"[ASYNC_DATA] 🔥 ASSIGNING TITLES TO VIDEOS (CRITICAL FIX)")
                    
                    # Get unassigned titles
                    unassigned_titles = [title for title in all_titles if title.assigned_to is None]
                    print(f"[ASYNC_DATA] Found {len(unassigned_titles)} unassigned titles")
                    ...
                    for i, video in enumerate(videos):
                        if i < len(unassigned_titles):
                            ...
                            print(f"[ASYNC_DATA] ✅ ASSIGNED title '{title.title[:50]}...' to video {i+1}")
                        else:
                            ...
                            if last_title:
                                ...
                                print(f"[ASYNC_DATA] ⚠️ Video {i+1} using last available title: '{last_title.title[:50]}...'")
                            else:
                                video.title_data = None
                                print(f"[ASYNC_DATA] ❌ Video {i+1} has no title available")
                else:
                    print(f"[ASYNC_DATA] ⚠️ No titles found for task")
                    for video in videos:
                        video.title_data = None
                
                return videos
            except Exception as e:
                print(f"[ASYNC_DATA] Error loading videos: {str(e)}")
                import traceback
                print(f"[ASYNC_DATA] Traceback: {traceback.format_exc()}")
                return []
        
        return await get_videos()
        
    except Exception as e:
        print(f"❌ [DATABASE] Error getting assigned videos: {str(e)}")
        return []
...
async def prepare_unique_videos_async(account_task, videos):
    ...
    try:
        ...
        print(f"🎬 [ASYNC_UNIQUIFY] Starting video uniquification for account")
        ...
        for i, video in enumerate(videos):
            try:
                if not (hasattr(video, 'video_file') and video.video_file):
                    print(f"⚠️ [ASYNC_UNIQUIFY] Video {i+1} has no video_file, skipping")
                    continue
                
                # Create temporary file from original
                video_filename = os.path.basename(video.video_file.name)
                print(f"📋 [ASYNC_UNIQUIFY] Preparing video: {video_filename}")
                ...
                def create_temp_file():
                    ...
                    print(f"✅ [ASYNC_UNIQUIFY] Created temp file: {temp_path}")
                    return temp_path
                ...
                if os.path.exists(temp_original) and os.path.getsize(temp_original) > 0:
                    ...
                    print(f"✅ [ASYNC_UNIQUIFY] Verified temp file: {os.path.basename(temp_original)} ({os.path.getsize(temp_original)} bytes)")
                else:
                    print(f"❌ [ASYNC_UNIQUIFY] Temp file verification failed: {temp_original}")
                    if os.path.exists(temp_original):
                        print(f"⚠️ [ASYNC_UNIQUIFY] File exists but size is {os.path.getsize(temp_original)} bytes")
                    else:
                        print(f"⚠️ [ASYNC_UNIQUIFY] File does not exist")
                    continue
                
                print(f"✅ [ASYNC_UNIQUIFY] Prepared video {i+1}/{len(videos)}: {os.path.basename(temp_original)}")
                
            except Exception as e:
                print(f"❌ [ASYNC_UNIQUIFY] Error preparing video {i+1}: {str(e)}")
                import traceback
                print(f"🔍 [ASYNC_UNIQUIFY] Traceback: {traceback.format_exc()}")
                continue
        
        print(f"🎯 [ASYNC_UNIQUIFY] Prepared {len(video_files)} videos for account")
        
        # Final verification - check all files exist
        for i, file_path in enumerate(video_files):
            if not os.path.exists(file_path):
                print(f"❌ [ASYNC_UNIQUIFY] CRITICAL: File {i+1} does not exist: {file_path}")
            else:
                print(f"✅ [ASYNC_UNIQUIFY] File {i+1} verified: {os.path.basename(file_path)} ({os.path.getsize(file_path)} bytes)")
        
        return video_files
        
    except Exception as e:
        print(f"❌ [ASYNC_UNIQUIFY] Critical error preparing videos: {str(e)}")
        import traceback
        print(f"🔍 [ASYNC_UNIQUIFY] Full traceback: {traceback.format_exc()}")
        return []
...
async def update_task_status_async(task_id, status, results=None):
    ...
    try:
        ...
        print(f"❌ [DATABASE] Error updating task status: {str(e)}")
        return False
...
async def handle_reels_dialog_async(page):
    ...
    try:
        print("[ASYNC_REELS_DIALOG] 🔍 Checking for Reels dialog...")
            ...
            try:
                ...
                if dialog_element and await dialog_element.is_visible():
                    ...
                    if dialog_text and any(keyword in dialog_text for keyword in ['Reels', 'видео', 'Теперь', 'Now', 'общедоступный', 'public']):
                        print(f"[ASYNC_REELS_DIALOG] ✅ Found Reels dialog with selector: {selector}")
                        print(f"[ASYNC_REELS_DIALOG] 📝 Dialog text preview: {dialog_text[:100]}...")
                        dialog_found = True
                        break
            except Exception as e:
                continue
        ...
        if not dialog_found:
            print("[ASYNC_REELS_DIALOG] ⚠️ No Reels dialog found, continuing...")
            return True
            ...
            try:
                ...
                if ok_button and await ok_button.is_visible():
                    ...
                    if button_text and ('OK' in button_text.upper() or 'ОК' in button_text.upper()):
                        print(f"[ASYNC_REELS_DIALOG] ✅ Found OK button with selector: {selector} (text: {button_text})")
                        break
                    else:
                        ok_button = None
            except Exception as e:
        ...
        if ok_button:
            ...
            print("[ASYNC_REELS_DIALOG] ✅ Clicked OK button")
            ...
            try:
                ...
                if not still_visible:
                    print("[ASYNC_REELS_DIALOG] ✅ Dialog closed successfully")
                else:
                    print("[ASYNC_REELS_DIALOG] ⚠️ Dialog still visible after click")
            except:
                print("[ASYNC_REELS_DIALOG] ✅ Dialog closed (element no longer accessible)")
            
            return True
        else:
            print("[ASYNC_REELS_DIALOG] ❌ Found dialog but no OK button")
            ...
            try:
                ...
                if any_button:
                    ...
                    print(f"[ASYNC_REELS_DIALOG] ⚠️ Clicking any button in dialog: {button_text}")
        ...
        print(f"[ASYNC_REELS_DIALOG] ❌ Error handling Reels dialog: {str(e)}")
...
async def verify_page_elements_state_async(page) -> bool:
    ...
    try:
        print("[ASYNC_UPLOAD] 🔍 Quick page elements verification...")
        
        # Quick check: document ready state
        ready_state = await page.evaluate("document.readyState")
        if ready_state != 'complete':
            print(f"[ASYNC_UPLOAD] ⚠️ Document not ready: {ready_state}")
            await asyncio.sleep(2)  # Faster check interval
            return False
        ...
        if not nav_element:
            print("[ASYNC_UPLOAD] ⚠️ No navigation element found")
            return False
        
        # Quick check: upload button
        upload_button = await page.query_selector('[aria-label*="New post"], [aria-label*="Create"], [data-testid="new-post-button"]')
        if not upload_button or not await upload_button.is_visible():
            print("[ASYNC_UPLOAD] ⚠️ Upload button not found or not visible")
            return False
        
        print("[ASYNC_UPLOAD] ✅ Page elements verified successfully")
        return True
        
    except Exception as e:
        print(f"[ASYNC_UPLOAD] ❌ Error in verify_page_elements_state_async: {str(e)}")
        return False
    ...
    try:
        ...
        def update_last_used():
            try:
                ...
                print(f"❌ [DATABASE] Account not found: {username}")
                return False
            except Exception as e:
                print(f"❌ [DATABASE] Error updating last_used: {str(e)}")
                return False
