...
class Upload:
    ...
    def upload_video(self, video: str, title=None, location=None, mentions=None):
        ...
        """
        page = self.page
        logger.info(f'🎬 Начинаем загрузку видео: {video}')
        ...
        if title:
            ...
            logger.info(f'📝 Используем стандартный заголовок: {title}')
            
        if location:
            location = location.encode('utf-8', 'ignore').decode('utf-8')
            logger.info(f'📍 Локация: {location}')
        else:
            logger.info('📍 Локация не указана')
        
        if mentions:
            logger.info(f'👥 Упоминания: {", ".join(mentions)}')
        else:
            logger.info('👥 Упоминания не указаны')

        logger.info('⏳ Начальная задержка перед загрузкой...')
        random_delay("major")
        ...
        try:
            ...
            try:
                ...
                if new_post_button.is_visible(timeout=config['implicitly_wait'] * 1000):
                    ...
                    logger.info('⏳ Ожидание после нажатия кнопки нового поста...')
                    time.sleep(3)
                else:
                    raise Exception("Кнопка не найдена по стандартному XPath")
            except Exception as e:
                logger.info(f'⚠️ Стандартная кнопка не найдена, пробуем альтернативные методы: {str(e)}')
                ...
                try:
                    ...
                    if create_button.is_visible(timeout=config['implicitly_wait'] * 1000):
                        ...
                        create_button.click()
                        logger.info('⏳ Ожидание после нажатия кнопки Create...')
                        time.sleep(3)
                    else:
                        raise Exception("Кнопка не найдена по роли")
                except Exception as e:
                    logger.info(f'⚠️ Кнопка Create не найдена по роли, пробуем дальше: {str(e)}')
                    ...
                    try:
                        ...
                        if create_aria_button.is_visible(timeout=config['implicitly_wait'] * 1000):
                            ...
                            logger.info('⏳ Ожидание после нажатия кнопки New post...')
                            time.sleep(3)
                        else:
                            raise Exception("Кнопка не найдена по aria-label")
                    except Exception as e:
                        logger.info(f'⚠️ Кнопка не найдена по aria-label, пробуем дальше: {str(e)}')
                        ...
                        try:
                            ...
                            if create_svg.is_visible(timeout=config['implicitly_wait'] * 1000):
                                ...
                                logger.info('⏳ Ожидание после нажатия на SVG иконку...')
                                time.sleep(3)
                            else:
                                raise Exception("SVG иконка не найдена")
                        except Exception as e:
                            logger.info(f'⚠️ SVG иконка не найдена, пробуем еще варианты: {str(e)}')
                            ...
                            try:
                                ...
                                if create_text.is_visible(timeout=config['implicitly_wait'] * 1000):
                                    ...
                                    create_text.click()
                                    logger.info('⏳ Ожидание после нажатия на текст "Create"...')
                                    time.sleep(3)
                                else:
                                    raise Exception('Текст "Create" не найден')
                            except Exception as e:
                                logger.info(f'⚠️ Текст "Create" не найден: {str(e)}')
                                ...
                                try:
                                    ...
                                    logger.info('⏳ Ожидание после перехода на URL создания поста...')
                                    time.sleep(5)
                                except Exception as e:
                                    logger.error(f'❌ Не удалось перейти на URL создания поста: {str(e)}')
            ...
            try:
                ...
                if alternate_post_button.is_visible(timeout=config['implicitly_wait'] * 1000):
                    ...
                    logger.info('⏳ Ожидание после нажатия альтернативной кнопки поста...')
                    time.sleep(3)
            except Exception as e:
                logger.info(f'⚠️ Альтернативная кнопка не найдена, продолжаем: {str(e)}')
                pass
            
            logger.info('⏳ Ожидание загрузки интерфейса...')
            time.sleep(5)
            ...
            try:
                ...
                if select_from_device.is_visible(timeout=config['implicitly_wait'] * 1000):
                    ...
                    if upload_input.count() > 0:
                        ...
                        logger.info('⏳ Ожидание после загрузки файла...')
                        time.sleep(5)
                    else:
                        ...
                        logger.info('⏳ Ожидание после нажатия кнопки "Select from device"...')
                        time.sleep(3)
                        ...
                        if upload_input.count() > 0:
                            ...
                            logger.error('❌ Не удалось найти элемент для загрузки файла')
                else:
                    logger.info('ℹ️ Кнопка "Select from device" не найдена, используем стандартный метод загрузки')
            except Exception as e:
                logger.info(f'⚠️ Кнопка "Select from device" не найдена, продолжаем: {str(e)}')
                
                # Стандартный метод загрузки файла
                logger.info('🔍 Поиск стандартного элемента загрузки файла...')
                upload_input = page.locator("input[type='file']")
                logger.info('⏳ Ожидание появления элемента загрузки файла...')
            ...
            try:
                ...
                if ok_button.is_visible(timeout=config['implicitly_wait'] * 1000):
                    ...
                    ok_button.click()
                    logger.info('⏳ Ожидание после нажатия кнопки OK...')
                    time.sleep(3)
            except Exception as e:
                logger.info(f'⚠️ Диалог подтверждения не найден, продолжаем: {str(e)}')
                pass
            ...
            try:
                ...
                try:
                    ...
                    logger.info('⏳ Ожидание появления элемента выбора кадрирования (XPath)...')
                    if select_crop.is_visible(timeout=config['implicitly_wait'] * 1000):
                        logger.info('👆 Открытие меню кадрирования (XPath)')
                        select_crop.click()
                        logger.info('⏳ Ожидание после открытия меню кадрирования...')
                        time.sleep(3)
                    else:
                        raise Exception("Элемент не найден через XPath")
                except Exception as e:
                    logger.info(f'⚠️ Не удалось найти элемент кадрирования через XPath: {str(e)}')
                    ...
                    try:
                        ...
                        logger.info(f'⚠️ Не удалось найти элемент кадрирования по тексту: {str(e)}')
                        ...
                        try:
                            ...
                            logger.info(f'⚠️ Не удалось найти элемент кадрирования по роли: {str(e)}')
                            
                            # Если все методы не сработали, пробуем пропустить этот шаг
                            logger.info('⚠️ Не удалось найти элемент кадрирования, пропускаем этот шаг')
                ...
                try:
                    ...
                    logger.info('⏳ Ожидание появления опции оригинального кадрирования (XPath)...')
                    if original_crop.is_visible(timeout=config['implicitly_wait'] * 1000):
                        logger.info('👆 Выбор оригинального кадрирования (XPath)')
                        original_crop.click()
                        logger.info('⏳ Ожидание после выбора оригинального кадрирования...')
                        time.sleep(2)
                    else:
                        raise Exception("Элемент не найден через XPath")
                except Exception as e:
                    logger.info(f'⚠️ Не удалось найти опцию оригинального кадрирования через XPath: {str(e)}')
                    ...
                    try:
                        ...
                        logger.info(f'⚠️ Не удалось найти опцию оригинального кадрирования по тексту: {str(e)}')
                        ...
                        try:
                            ...
                            logger.info(f'⚠️ Не удалось найти опцию оригинального кадрирования по роли: {str(e)}')
                ...
                try:
                    ...
                    logger.info(f'⚠️ Не удалось закрыть меню кадрирования: {str(e)}')
                    # Пробуем нажать Escape для закрытия меню
                    page.keyboard.press("Escape")
                
                logger.info('⏳ Ожидание после настройки кадрирования...')
                time.sleep(3)
                
            except Exception as e:
                logger.info(f'⚠️ Пропущена настройка кадрирования: {str(e)}')
            
            # Navigate through the upload steps
            for i in range(2):
                logger.info(f'⏭️ Переход к следующему шагу ({i+1}/2)...')
                self._next_page()
                logger.info(f'⏳ Ожидание после перехода к шагу {i+1}...')
                time.sleep(4)
            
            # Add description
            logger.info(f'📝 Ввод описания: {title}')
            try:
                logger.info('🔍 Поиск поля для ввода описания...')
                description_field = page.locator("xpath=" + config['selectors']['upload']['description_field'])
                logger.info('⏳ Ожидание появления поля для ввода описания...')
                ...
                logger.error(f'❌ Ошибка при вводе описания: {str(e)}')
            
            # Add location if specified
            if location:
                logger.info(f'📍 Ввод локации: {location}')
                logger.info('🔍 Поиск поля для ввода локации...')
                location_field = page.locator("xpath=" + config['selectors']['upload']['location_field'])
                logger.info('⏳ Ожидание появления поля для ввода локации...')
                ...
                logger.info('⏳ Ожидание появления локации в списке...')
                first_location.wait_for(state="visible", timeout=config['explicit_wait'] * 1000)
                logger.info('👆 Выбор локации')
                first_location.click()
                logger.info('⏳ Ожидание после выбора локации...')
                time.sleep(3)
            
            # Add mentions if specified
            if mentions:
                logger.info(f'👥 Добавление упоминаний: {", ".join(mentions)}')
                for mention in mentions:
                    ...
                    logger.info('⏳ Ожидание после ввода имени пользователя...')
                    time.sleep(3)
                    ...
                    logger.info('⏳ Ожидание появления пользователя в списке...')
                    first_mention.wait_for(state="visible", timeout=config['explicit_wait'] * 1000)
                    logger.info('👆 Выбор пользователя')
                    first_mention.click()
                    logger.info('⏳ Ожидание после выбора пользователя...')
                    time.sleep(2)
                
                logger.info('✅ Завершение добавления упоминаний')
                ...
                done_btn.click()
                logger.info('⏳ Ожидание после нажатия кнопки "Готово"...')
                time.sleep(3)
            
            # Post the video
            logger.info('🚀 Публикация видео...')
            ...
            logger.info('⏳ Длительное ожидание процесса публикации...')
            ...
            try:
                ...
                logger.info('⏳ Ожидание появления индикатора успешной публикации...')
                is_posted.wait_for(state="visible", timeout=config['explicit_wait'] * 1000)
                logger.info(f'✅ Видео {video} успешно опубликовано!')
            except Exception as e:
                logger.error(f'❌ Видео {video} не опубликовано. Проверьте наличие ошибок: {str(e)}')
            
            # Refresh the page
            logger.info('🔄 Обновление страницы...')
            page.reload()
            logger.info('⏳ Ожидание после обновления страницы...')
            time.sleep(5)
            
            return True
        except Exception as e:
            logger.error(f'❌ Ошибка при загрузке видео {video}: {str(e)}')
            return False
    ...
    def _next_page(self):
        ...
        try:
            ...
            if next_button.is_visible(timeout=config['implicitly_wait'] * 1000):
                ...
                next_button.click()
                logger.info('⏳ Ожидание после нажатия кнопки "Далее"...')
                time.sleep(5)  # Увеличиваем время ожидания после нажатия кнопки
                return True
        except Exception as e:
            logger.info(f'⚠️ Не удалось найти кнопку "Далее" по XPath: {str(e)}')
        ...
        try:
            ...
            logger.info(f'⚠️ Не удалось найти кнопку "Далее" по тексту: {str(e)}')
        ...
        try:
            ...
            logger.info(f'⚠️ Не удалось найти кнопку "Далее" по роли: {str(e)}')
        ...
        try:
            ...
            logger.info(f'⚠️ Не удалось найти кнопку "Далее" по aria-label: {str(e)}')
        ...
        try:
            ...
            logger.info('⏳ Ожидание после нажатия Enter...')
            time.sleep(5)
            return True
        except Exception as e:
            logger.error(f'❌ Все методы поиска кнопки "Далее" не сработали: {str(e)}')
            return False
    ...
    def upload_videos(self, videos: list[dict]):
        ...
        for i, video in enumerate(videos):
            try:
                logger.info(f'🎬 Загрузка видео {i+1}/{len(videos)}')
                ...
                if not result:
                    logger.error(f'❌ Не удалось загрузить видео {i+1}/{len(videos)}')
                    success = False
                else:
                    logger.info(f'✅ Видео {i+1}/{len(videos)} успешно загружено')
                    
                # Добавляем случайную задержку между загрузками видео
                if i < len(videos) - 1:  # Если это не последнее видео
                    logger.info('⏳ Ожидание перед загрузкой следующего видео...')
                    random_delay("major")
            except Exception as e:
                logger.error(f'❌ Ошибка при загрузке видео {i+1}/{len(videos)}: {str(e)}')
                success = False
        ...
        if success:
            ...
            logger.warning('⚠️ Некоторые видео не были загружены')
